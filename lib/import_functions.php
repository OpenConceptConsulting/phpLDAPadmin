    ...
    public function accept()
    {
        if (get_request('ldif', 'REQUEST')) {
            ...
            system_message(
                ...
                sprintf('cmd.php?cmd=import_form&server_id=%s', get_request('server_id', 'REQUEST'))
            );
        ...
        switch ($template->getType()) {
            ...
            case 'modrdn':
                return $server->rename(
                    $template->getDN(),
                    $template->modrdn['newrdn'],
                    $template->modrdn['newsuperior'],
                    $template->modrdn['deleteoldrdn']
                );
    ...
    private function getModifyDetails($lines)
    {
        ...
        while (count($lines)) {
            ...
            if (!in_array($action_attribute, array('add', 'delete', 'replace'))) {
                return $this->error(
                    _('Missing modify command add, delete or replace'),
                ...
                case 'delete':
                    ...
                    if (is_null($attribute)) {
                        return $this->error(
                            sprintf(
                                '%s %s',
                                _('Attempting to delete a non existant attribute'),
                ...
                case 'replace':
                    ...
                    if (is_null($attribute)) {
                        return $this->error(
                            sprintf(
                                '%s %s',
                                _('Attempting to replace a non existant attribute'),
                                $action_attribute_value
                            ),
                            array_merge(array($currentLine), $lines)
                        );
                ...
                if (preg_match('/:/', $currentLine)) {
                    ...
                    if ($attr == $action_attribute_value) {
                        ...
                        switch ($action_attribute) {
                            case 'add':
                                if (is_null($attribute)) {
                                    $attribute = $this->template->addAttribute(
                                        $attr,
                                        array('values' => array($attribute_value_part))
                                    );
                                } else {
                            ...
                            case 'delete':
                                ...
                                if (($key = array_search($attribute_value_part, $attribute->getValues())) !== false) {
                                    ...
                                    return $this->error(
                                        sprintf('%s %s', _('Delete value doesnt exist in DN'), $attribute_value_part),
                                        array_merge(array($currentLine), $lines)
                                    );
                                }
                        ...
                        return $this->error(
                            sprintf(
                                '%s %s',
                                _('The attribute to modify doesnt match the one specified by'),
                                $action_attribute
                            ),
                            array_merge(array($currentLine), $lines)
                        );
                    ...
                    return $this->error(
                        sprintf('%s %s', _('Attribute not valid'), $currentLine),
                        array_merge(array($currentLine), $lines)
                    );
                }
