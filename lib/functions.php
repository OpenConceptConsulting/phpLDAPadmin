        ...
        system_message(
            array(
                ...
                'body' => sprintf(
                    '%s: %s [%s]',
                    __METHOD__,
                    _('Called to load a class that cant be found'),
                    $className
                ),
                'type' => 'error'
            )
...
function app_error_handler($errno, $errstr, $file, $lineno)
{
    ...
    if ($errno == E_NOTICE) {
        ...
        $body .= sprintf(
            '<tr><td>%s:</td><td><b>%s</b> %s <b>%s</b>, %s <b>%s</b></td></tr>',
            _('File'),
            $file,
            _('line'),
            $lineno,
        ...
        $body .= sprintf(
            '<tr><td>Versions:</td><td>PLA: <b>%s</b>, PHP: <b>%s</b>, SAPI: <b>%s</b></td></tr>',
            app_version(),
            phpversion(),
            php_sapi_name()
        );
        $body .= sprintf(
            ...
            isset($_SERVER['SERVER_SOFTWARE']) ? $_SERVER['SERVER_SOFTWARE'] : 'SCRIPT'
        );

        if (function_exists('get_href')) {
            $body .= sprintf(
                '<tr><td colspan="2"><a href="%s" onclick="target=\'_blank\';"><center>%s.</center></a></td></tr>',
...
function auto_lang($lang)
{
    switch ($lang) {
        ...
        case 'cs':
            return 'cs_CZ';
        default:
            return sprintf('%s_%s', $lang, strtoupper($lang));
    }
}
...
function check_config($config_file)
{
    ...
    if (strcmp(phpversion(), REQUIRED_PHP_VERSION) < 0) {
        system_message(
            array(
                ...
                'body' => sprintf(
                    'phpLDAPadmin requires PHP version %s or greater.<br /><small>(You are using %s)</small>',
    ...
    if (!isset($servers) || count($servers->GetServerList()) == 0) {
        error(
            _('Your config.php is missing Server Definitions. Please see the sample file config/config.php.example.'),
            'error',
            'index.php',
            true
        );
    ...
    if ((ini_get('memory_limit') > -1) && ini_get('memory_limit') < $config->getValue('session', 'memorylimit')) {
        system_message(
            array(
                ...
                'body' => sprintf(
                    'Your php memory limit is low - currently %s, you should increase it to atleast %s. This is normally controlled in /etc/php.ini.',
                    ini_get('memory_limit'),
                    $config->getValue('session', 'memorylimit')
                ),
                'type' => 'error'
            )
        );
        ...
        case 'main' :
            return array(
                ...
                'purge' => array(
                    ...
                    'enable' => isset($_SESSION[APPCONFIG]) ?
                        $_SESSION[APPCONFIG]->isCommandAvailable('script', 'purge_cache') : false,
                    'link' => sprintf(
                        'href="cmd.php?cmd=purge_cache" onclick="return ajDISPLAY(\'BODY\',\'cmd=purge_cache\',\'%s\');" title="%s"',
                        _('Clearing cache'),
                        _('Purge caches')
                    ),
                ...
                'hide_debug_info' => array(
                    'title' => _('Show Cache'),
                    'enable' => isset($_SESSION[APPCONFIG]) ?
                        $_SESSION[APPCONFIG]->isCommandAvailable('script', 'show_cache') : false,
                    'link' => sprintf(
                        'href="cmd.php?cmd=show_cache" onclick="return ajDISPLAY(\'BODY\',\'cmd=show_cache\',\'%s\');" title="%s"',
                        _('Loading'),
                        _('Show Cache'),
                        _('Show Cache')
                    ),
        ...
        case 'top' :
            return array(
                'forum' => array(
                    'title' => _('Forum'),
                    'enable' => isset($_SESSION[APPCONFIG]) ?
                        $_SESSION[APPCONFIG]->isCommandAvailable('cmd', 'oslinks') : true,
                    'link' => sprintf(
                        'href="%s" title="%s" onclick="target=\'_blank\';"',
                        get_href('forum'),
                        _('Forum')
                    ),
                ...
                'feature' => array(
                    ...
                    'image' => sprintf(
                        '<img src="%s/request-feature-big.png" alt="%s" />',
                        IMGDIR,
                        _('Request feature')
                    )
                ),
...
function debug_log($msg, $level, $indent)
{
    ...
    if ($_SESSION[APPCONFIG]->getValue('debug', 'addr')) {
        if (isset($_SERVER['HTTP_X_FORWARDED_FOR']) && $_SERVER['HTTP_X_FORWARDED_FOR'] == $_SESSION[APPCONFIG]->getValue(
                'debug',
                'addr'
            )
        ) {
    ...
    if ($debug_file || $_SESSION[APPCONFIG]->getValue('debug', 'file')) {
        if (!$debug_file) {
            $debug_file = fopen(
                $_SESSION[APPCONFIG]->getValue('debug', 'file'),
...
function error($msg, $type = 'note', $redirect = null, $fatal = false, $backtrace = false)
{
    ...
    if ($backtrace) {
        ...
        foreach (debug_backtrace() as $error => $line) {
            ...
            $body .= sprintf(
                '<tr class="hightlight"><td colspan="2"><b><small>%s</small></b></td><td>%s (%s)</td></tr>',
                _('File'),
                isset($line['file']) ? $line['file'] : $last['file'],
                isset($line['line']) ? $line['line'] : ''
            );

            $_SESSION['backtrace'][$error]['function'] = $line['function'];
            $body .= sprintf(
                '<tr><td>&nbsp;</td><td><b><small>%s</small></b></td><td><small>%s',
                _('Function'),
                $line['function']
            );

            if (isset($line['args'])) {
                $display = strlen(serialize($line['args'])) < 50 ? htmlspecialchars(serialize($line['args'])) :
                    htmlspecialchars(substr(serialize($line['args']), 0, 50)) . '...<TRUNCATED>';
                $_SESSION['backtrace'][$error]['args'] = $line['args'];
                if (file_exists(LIBDIR . '../tools/unserialize.php')) {
                    $body .= sprintf(
                        '&nbsp;(<a href="%s?index=%s" onclick="target=\'backtrace\';">%s</a>)',
...
function get_request($attr, $type = 'POST', $die = false, $default = null)
{
    switch ($type) {
        case 'GET':
            $value = isset($_GET[$attr]) ?
                (is_array($_GET[$attr]) ? $_GET[$attr] :
                    (empty($_GET['nodecode'][$attr]) ? rawurldecode($_GET[$attr]) : $_GET[$attr])) : $default;
            break;

        case 'REQUEST':
            $value = isset($_REQUEST[$attr]) ?
                (is_array($_REQUEST[$attr]) ? $_REQUEST[$attr] :
                    (empty($_REQUEST['nodecode'][$attr]) ? rawurldecode($_REQUEST[$attr]) : $_REQUEST[$attr])) :
                $default;
            break;

        case 'SESSION':
            $value = isset($_SESSION[$attr]) ?
                (is_array($_SESSION[$attr]) ? $_SESSION[$attr] :
                    (empty($_SESSION['nodecode'][$attr]) ? rawurldecode($_SESSION[$attr]) : $_SESSION[$attr])) :
                $default;
            break;

        case 'POST':
        default:
            $value = isset($_POST[$attr]) ?
                (is_array($_POST[$attr]) ? $_POST[$attr] :
                    (empty($_POST['nodecode'][$attr]) ? rawurldecode($_POST[$attr]) : $_POST[$attr])) : $default;
            break;
    }
    ...
    if ($die && is_null($value)) {
        system_message(
            array(
                'title' => _('Generic Error'),
                'body' => sprintf(
                    '%s: Called "%s" without "%s" using "%s"',
                    basename($_SERVER['PHP_SELF']),
                    get_request('cmd', 'REQUEST'),
                    $attr,
                    $type
                ),
                'type' => 'error'
 ...
 *
 * @return object|null Returns the cached attributed requested,
...
function obfuscate_password_display($enc = null)
{
    ...
    if ($_SESSION[APPCONFIG]->getValue('appearance', 'obfuscate_password_display')) {
        $return = true;
    } elseif (!$_SESSION[APPCONFIG]->getValue('appearance', 'show_clear_password') && (is_null(
                $enc
            ) || $enc == 'clear')
    ) {
        $return = true;
...
function get_next_number($base, $attr, $increment = false, $filter = false, $startmin = null)
{
    ...
    if (!$server->getValue('auto_number', 'enable')) {
        system_message(
            array(
                ...
                'body' => sprintf(
                    '%s (<b>%s</b>)',
                    _('A call was made to get_next_number(), however, it is disabled for this server'),
                    $attr
                ),
                'type' => 'warn'
            )
    ...
    if (!$server->login(
        $server->getValue('auto_number', 'dn'),
        $server->getValue('auto_number', 'pass'),
        'auto_number'
    )
    ) {
        system_message(
            array(
                ...
                'body' => sprintf(
                    '%s (<b>%s</b>)',
                    _(
                        'Unable to connect to LDAP server with the auto_number login/password, please check your configuration.'
                    ),
                    $server->getName()
                ),
                'type' => 'warn'
        ...
        system_message(
            array(
                ...
                'body' => sprintf(
                    '%s (<b>%s</b>)',
                    _(
                        'A call was made to get_next_number(), however, the base to search does not exist for this server.'
                    ),
                    $query['base']
                ),
                'type' => 'warn'
        ...
        system_message(
            array(
                ...
                'body' => sprintf(
                    '%s (<b>%s</b>)',
                    _(
                        'The search attribute for AUTO_NUMBER is invalid, expecting a single valid attribute.'
                    ),
                    $attr
                ),
                'type' => 'warn'
                    ...
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s (<b>%s</b>)',
                                _(
                                    'Please change your filter parameter, or check your auto_number,search_base configuration'
                                ),
                                $query['filter']
                            ),
                            'type' => 'warn'
        ...
        default:
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        '%s (<b>%s</b>)',
                        _('Your config file specifies an unknown AUTO_NUMBER search mechanism.'),
                        $server->getValue('auto_number', 'mechanism')
                    ),
                    'type' => 'warn'
                )
            );
 ...
 * @param int $include_attrs If true, include attributes in the RDN string. See http://php.net/ldap_explode_dn for details
 * @param bool $decode
...
function pla_verbose_error($key)
{
    ...
    if (!count($CACHE)) {
        ...
        $entries = array();
        preg_match_all(
            "/0x[A-Fa-f0-9][A-Za-z0-9]\s+[0-9A-Za-z_]+\s+\"[^\"]*\"\n/",
            $contents,
            $entries
        );
    ...
    if (!count($CACHE)) {
        ...
        foreach ($entries[0] as $values) {
            ...
            if ($oid_id) {
                ...
                $desc = isset($entry[8]) ? $entry[8] :
                    sprintf('<acronym title="%s">%s</acronym>', $unknown['desc'], $unknown['title']);
 ...
 */
function draw_jpeg_photo(
    $server,
    $dn,
    $attr_name = 'jpegphoto',
    $index,
    $draw_delete_buttons = false,
    $options = array()
) {
    if (DEBUG_ENABLED && (($fargs = func_get_args()) || $fargs = 'NOARGS')) {
        debug_log('Entered (%%)', 1, 0, __FILE__, __LINE__, __METHOD__, $fargs);
    ...
    if (!isset($jpeg_data[$attr_name][$index]) || !$jpeg_data[$attr_name][$index]) {
        system_message(
            array(
                ...
                'body' => sprintf(
                    '%s %s',
                    _('Could not fetch jpeg data for attribute'),
                    $attr_name
                ),
                'type' => 'warn'
            )
            ...
            if (!$outjpeg) {
                system_message(
                    array(
                        ...
                        'body' => sprintf(
                            _('Please check jpeg,tmpdir is a writable directory in the phpLDAPadmin config.php'),
                            $jpeg_temp_dir
                        ),
                        'type' => 'warn'
                    )
    ...
    if ($draw_delete_buttons) # <!-- JavaScript function deleteJpegPhoto() to be defined later by calling script -->
    {
        printf(
            '<br/><a href="javascript:deleteAttribute(\'%s\');" style="color:red; font-size: 75%%">%s</a>',
            $attr_name,
            _('Delete photo')
        );
    }
    ...
    switch ($enc_type) {
        ...
        case 'sha':
            ...
            if (function_exists('sha1')) {
                ...
                error(
                    _('Your PHP install does not have the mhash() function. Cannot do SHA hashes.'),
                    'error',
                    'index.php'
                );
            }
        ...
        case 'sha512':
            if (function_exists('openssl_digest') && function_exists('base64_encode')) {
                ...
                error(
                    _(
                        'Your PHP install doest not have the openssl_digest() or base64_encode() function. Cannot do SHA512 hashes. '
                    ),
                    'error',
                    'index.php'
                );
    ...
    switch ($cypher) {
        ...
        case 'crypt':
            ...
            elseif (strstr($cryptedpassword, '_')) {
                ...
                if (!defined('CRYPT_EXT_DES') || CRYPT_EXT_DES == 0) {
                    error(
                        _('Your system crypt library does not support extended DES encryption.'),
                        'error',
                        'index.php'
                    );
                }
...
function draw_chooser_link($form, $element, $include_choose_text = true, $rdn = 'none')
{
    ...
    $href = sprintf(
        "javascript:dnChooserPopup('%s','%s','%s');",
        $form,
        $element,
        $rdn == 'none' ? '' : rawurlencode($rdn)
    );
 ...
 * @param int $with_attributes (optional) Whether to include attribute names (see http://php.net/ldap_explode_dn for details)
...
function pla_explode_dn($dn, $with_attributes = 0)
{
    ...
    if (isset($CACHE['explode'][$dn][$with_attributes])) {
        if (DEBUG_ENABLED) {
            debug_log(
                'Return CACHED result (%s) for (%s)',
                1,
                0,
                __FILE__,
                __LINE__,
                __METHOD__,
                $CACHE['explode'][$dn][$with_attributes],
                $dn
            );
        }
...
function get_href($type, $extra_info = '')
{
    ...
    switch ($type) {
        ...
        case 'logo':
            ...
            return isset($_SESSION) && !$_SESSION[APPCONFIG]->getValue('appearance', 'remoteurls') ? '' :
...
function server_select_list($selected = null, $logged_on = false, $name = 'index', $isVisible = true, $js = null)
{
    ...
    foreach ($_SESSION[APPCONFIG]->getServerList($isVisible) as $index => $server) {
        if ($server->isVisible()) {
            ...
            $server_menu_html .= sprintf(
                ...
                $server->getIndex(),
                ($server->getIndex() == $selected ? 'selected="selected"' : ''),
                $server->getName()
            );
    ...
    if ($count > 1) {
        ...
        return sprintf(
            ...
            $selected_server->getName(),
            $name,
            $selected_server->getIndex()
        );
    } else {
        return '';
    }
