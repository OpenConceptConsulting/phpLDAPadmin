...
class QueryRender extends PageRender
{
    ...
    protected function drawTemplateChoice()
    {
        ...
        echo '<tr>';
        printf(
            '<td><acronym title="%s">%s</acronym></td>',
            _('The format to show the query results'),
            _('Display Format')
        );
        echo '<td>';
        ...
        printf(
            ...
            $_SESSION[APPCONFIG]->getValue('search', 'display') == 'list' ? 'selected="selected"' : '',
            _('list')
        );
        printf(
            '<option value="table" %s>%s</option>',
            $_SESSION[APPCONFIG]->getValue('search', 'display') == 'table' ? 'selected="selected"' : '',
            _('table')
        );
        ...
        printf(
            '<td><input type="text" name="base" value="%s" style="width: 200px" id="base" />',
            count($baseDNs) == 1 ? $baseDNs[0] : ''
        );
        ...
        printf(
            '<td><acronym title="%s">%s</acronym></td>',
            htmlspecialchars(_('Standard LDAP search filter. Example: (&(sn=Smith)(givenName=David))')),
            _('Search Filter')
        );

        printf(
            '<td><input type="text" name="filter" id="filter" style="width: 200px" value="%s" /></td>',
            'objectClass=*'
        );
        ...
        printf(
            '<td><acronym title="%s">%s</acronym></td>',
            _('A list of attributes to display in the results (comma-separated)'),
            _('Show Attributes')
        );

        printf(
            '<td><input type="text" name="display_attrs" style="width: 200px" value="%s" /></td>',
        ...
        printf(
            '<td><acronym title="%s">%s</acronym></td>',
            _('Set the search results to 0 to retrieve all available records'),
            _('Search Results')
        );
        printf(
            '<td><input type="text" name="size_limit" id="size_limit" style="width: 200px" value="%s" /></td>',
            $_SESSION[APPCONFIG]->getValue('search', 'size_limit')
        );
        echo '</tr>';
        ...
        printf(
            '<tr><td colspan="2" style="text-align: center;"><br /><input type="submit" name="search" value="%s" /></td></tr>',
            _('Search')
        );
    ...
    private function visitEnd()
    {
        ...
        foreach ($this->template->results as $base => $results) {
            ...
            printf(
                '<div id="DN%s" style="display: %s">',
                $this->getAjaxRef($base),
                ($show == $this->getAjaxRef($base) ? 'block' : 'none')
            );
            ...
            switch (get_request('format', 'REQUEST', false, $_SESSION[APPCONFIG]->getValue('search', 'display'))) {
                case 'list':
                    foreach ($results as $dndetails) {
                        ...
                        printf(
                            '<td class="icon"><img src="%s/%s" alt="icon" /></td>',
                            IMGDIR,
                            get_icon($server->getIndex(), $dndetails['dn'])
                        );

                        printf(
                            '<td colspan="2"><a href="cmd.php?cmd=template_engine&amp;server_id=%s&amp;dn=%s">%s</a></td>',
                            $server->getIndex(),
                            $this->template->getDNEncode(),
                            htmlspecialchars(get_rdn($dndetails['dn']))
                        );
                        echo '</tr>';

                        printf(
                            '<tr class="list_item"><td class="blank">&nbsp;</td><td class="heading">dn</td><td class="value">%s</td></tr>',
                ...
                case 'table':
                    ...
                    foreach ($results as $dndetails) {
                        ...
                        printf(
                            '<tr class="%s" id="tr_ma_%s" onclick="var cb=document.getElementById(\'ma_%s\'); cb.checked=!cb.checked;">',
                            $j % 2 ? 'even' : 'odd',
                            $j,
                            $j
                        );
                        ...
                        if ($_SESSION[APPCONFIG]->getValue('mass', 'enabled')) {
                            printf(
                                '<td><input type="checkbox" id="ma_%s" name="dn[]" value="%s" onclick="this.checked=!this.checked;" /></td>',
                                $j,
                                $dndetails['dn']
                            );
                        }
                        ...
                        $href = sprintf(
                            'cmd=template_engine&server_id=%s&dn=%s',
                            $server->getIndex(),
                            $this->template->getDNEncode()
                        );
                        printf(
                            '<td class="icon"><a href="cmd.php?%s"><img src="%s/%s" alt="icon" /></a></td>',
                            ...
                            if ($attr == 'dn') {
                                $dn_display = strlen($dndetails['dn']) > 40
                                    ...
                                    sprintf(
                                        ...
                                        htmlspecialchars(substr($dndetails['dn'], 0, 40))
                                    )
                                    : htmlspecialchars($dndetails['dn']);
                    ...
                    if ($_SESSION[APPCONFIG]->getValue('mass', 'enabled')) {
                        ...
                        printf(
                            '<td><input type="checkbox" name="allbox" value="1" onclick="CheckAll(1,\'massform_\',%s);" /></td>',
                            $counter
                        );
                        ...
                        foreach ($mass_actions as $display => $action) {
                            printf(
                                '<button type="submit" name="cmd" value="%s">%s</button>&nbsp;&nbsp;',
                                $action,
                                $display
                            );
                        }
    ...
    private function getAFattrs()
    {
        ...
        foreach (explode(',', $this->template->getAttrDisplayOrder()) as $attr) {
            $results[strtolower($attr)] = $attribute_factory->newAttribute(
                $attr,
                array('values' => array()),
                $this->getServerID()
            );
    ...
    private function drawBaseTabs()
    {
        ...
        foreach ($this->template->results as $base => $results) {
            ...
            printf(
                '<td id="CTL%s" onclick="return ajSHOWTHIS(\'DN\',\'%s\',\'CTL\');" style="background-color: %s;">%s</td>',
                $this->getAjaxRef($base),
                $this->getAjaxRef($base),
    ...
    private function drawResultsTable($base, $results)
    {
        ...
        echo '<tr>';
        printf(
            '<td>%s: <b>%s</b><br/><br/><div class="execution_time">(%s %s)</div></td>',
            _('Entries found'),
            number_format($results),
            $this->template->resultsdata[$base]['time'],
            _('seconds')
        );

        if ($_SESSION[APPCONFIG]->isCommandAvailable('script', 'export') && $_SESSION[APPCONFIG]->isCommandAvailable(
                'script',
                'export_form'
            )
        ) {
            $href = htmlspecialchars(
                sprintf(
                    'cmd.php?cmd=export_form&server_id=%s&scope=%s&dn=%s&filter=%s&attributes=%s',
                    $server->getIndex(),
                    $this->template->resultsdata[$base]['scope'],
                    $base,
                    rawurlencode($this->template->resultsdata[$base]['filter']),
                    rawurlencode(implode(', ', $this->template->resultsdata[$base]['attrs']))
                )
            );

            printf(
                '<td style="text-align: right; width: 85%%"><small>[ <a href="%s"><img src="%s/save.png" alt="Save" /> %s</a> ]</small>',
                $href,
                IMGDIR,
                _('export results')
            );
        ...
        foreach (array('list', 'table') as $f) {
            ...
            if (get_request('format', 'REQUEST', false, $_SESSION[APPCONFIG]->getValue('search', 'display')) == $f) {
                ...
                $query_string = htmlspecialchars(
                    sprintf(
                        '%s&format=%s&show=%s&focusbase=%s',
                        array_to_query_string($_GET, array('format', 'meth')),
                        $f,
                        $this->getAjaxRef($base),
                        $base
                    )
                ...
                if (isAjaxEnabled()) {
                    printf(
                        '<a href="cmd.php?%s" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'%s\');">%s</a>',
                        $query_string,
                        $query_string,
                        _('Loading Search'),
                        _($f)
        ...
        printf(
            '<small>%s: <b>%s</b></small>',
            _('Filter performed'),
            htmlspecialchars($this->template->resultsdata[$base]['filter'])
        );
