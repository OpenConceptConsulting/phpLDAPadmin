        ...
        foreach ($properties as $index => $value) {
            ...
            if (isset($this->$index)) {
                ...
                debug_dump_backtrace(
                    sprintf('Unknown property (%s) with value (%s) for (%s)', $index, $value, $this->getName()),
                    1
                );
            }
        }
        ...
        if (is_array($values)) {
            foreach ($values as $index => $value) {
                switch ($index) {
                    ...
                    case 'helper':
                        if (!isset($values['post']) && !$_SESSION[APPCONFIG]->getValue(
                                'appearance',
                            ...
                            system_message(
                                array(
                                    'title' => sprintf(
                                        '%s [<i>%s</i>]',
                                        _('Missing [post] setting in XML file'),
                                        $index
                                    ),
                        ...
                        if (isset($value['value']) && !is_array($value['value']) && preg_match(
                                '/^=php\.(\w+)\((.*)\)$/',
                                $value['value'],
                                $matches
                            )
                                ...
                                if (!$_SESSION[APPCONFIG]->getValue('appearance', 'hide_template_warning')) {
                                    system_message(
                                        array(
                                            ...
                                            'body' => sprintf(
                                                '%s <small>[%s]</small>',
                                                _('Unknown XML type setting for helper will be ignored.'),
                                                $detail
                                            ),
                                            'type' => 'warn'
                                        )
                    ...
                    case 'hidden':
                        $value ? $this->visible = false : $this->visible = true;
                        break;

                    case 'spacer':
                        $value ? $this->$index = true : $this->$index = false;
                        break;
                    ...
                    case 'type':
                        if (!in_array(
                                $value,
                                array('password', 'multiselect', 'select', 'textarea')
                            ) && !$_SESSION[APPCONFIG]->getValue('appearance', 'hide_template_warning')
                        ) {
                            system_message(
                                array(
                                    ...
                                    'body' => sprintf(
                                        '%s <small>[%s]</small>',
                                        _('Unknown XML type setting will be ignored.'),
                                        $value
                                    ),
                                    'type' => 'warn'
                                )
                    ...
                    case 'value':
                                    ...
                                    system_message(
                                        array(
                                            ...
                                            'body' => sprintf(
                                                '%s <small>[%s]</small>',
                                                _(
                                                    'Template defines more values than can be accepted by attribute.'
                                                ),
                                                $this->getName(true)
                                            ),
                                            'type' => 'warn'
                        ...
                        } else # Check to see if the value is auto generated.
                        {
                            if (preg_match('/^=php\.(\w+)\((.*)\)$/', $value, $matches)) {
                        ...
                        if (!$_SESSION[APPCONFIG]->getValue('appearance', 'hide_template_warning')) {
                            system_message(
                                array(
                                    ...
                                    'body' => sprintf(
                                        '%s <small>[%s]</small>',
                                        _('Unknown attribute setting will be ignored.'),
                                        serialize($value)
                                    ),
                                    'type' => 'warn'
                                )
