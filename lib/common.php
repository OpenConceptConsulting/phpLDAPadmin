    ...
    if (is_dir(realpath(sprintf('images/%s', $_SESSION[APPCONFIG]->getValue('appearance', 'theme'))))
        && is_file(
            realpath(
                sprintf(
                    'css/%s/%s',
                    $_SESSION[APPCONFIG]->getValue('appearance', 'theme'),
...
if (isset($_SERVER['SERVER_SOFTWARE']) && !isset($_SESSION[APPCONFIG])) {
    ...
    if (!method_exists($_SESSION[APPCONFIG], 'CheckCustom')) {
        error(
            'Unknown situation, $_SESSION[APPCONFIG] exists, but method CheckCustom() does not',
            'error',
            null,
            true,
            true
...
if (DEBUG_ENABLED) {
    debug_log(
        'Application (%s) initialised and starting with (%s).',
        1,
        0,
        __FILE__,
        __LINE__,
...
if (@ini_get('safe_mode') && !in_array('LANG', explode(',', @ini_get('safe_mode_allowed_env_vars')))) {
    error(
        'You are running in SAFE_MODE, but LANG is not in the safe_mode_allowed_env_vars. Please add LANG to safe_mode_allowed_env_vars',
        'error',
        true,
        false
    );
...
if (!$app['server']->isSessionValid()) {
    system_message(
        array(
            ...
            'body' => sprintf(
                '%s %s %s',
                _('Your Session timed out after'),
                $app['server']->getValue('login', 'timeout'),
                _('min. of inactivity. You have been automatically logged out.')
            ),
            'type' => 'info'
        ),
        sprintf('index.php?server_id=%s&refresh=SID_%s', $app['server']->getIndex(), $app['server']->getIndex())
    );
