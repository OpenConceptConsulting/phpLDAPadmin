...
abstract class Tree
{
    ...
    /**
     * @var int Server that this tree represents
     */
    private $server_id = null;
    /**
     * @var TreeItem[] List of entries in the tree view cache
     */
     ...
     *
     * @return ldap_pla Server Object for this tree
     */
     ...
     *
     * @return TreeItem[] Base DN entries
     */
    public function getBaseEntries()
    {
        ...
        foreach ($this->entries as $details) {
            if ($details->isBaseDN() AND ((!$this->getServer()->getValue(
                        'server',
                        'hide_noaccess_base'
                    )) OR $details->isInLdap())
            ) {
    ...
    /**
     * This function will take the DN, convert it to lowercase and strip unnecessary
     ...
     * are in fact in the same branch, but PLA will show them inconsistently.
     *
     * @param string $dn DN to clean
     * @return string Lowercase clean DN
     */
     ...
     * Get a tree entry
     *
     * @param string $dn DN to retrieve
     * @return TreeItem Tree DN object
     */
     ...
     *
     * @param string $dnOLD Old DN
     * @param string $dnNEW New DN
     */
     ...
     * @param boolean $nolimit LDAP Size Limit
     */
    public function readChildren($dn, $nolimit = false)
    {
        ...
        $ldap['children'] = $server->getContainerContents(
            $dn,
            null,
            $ldap['child_limit'],
            $ldap['filter'],
