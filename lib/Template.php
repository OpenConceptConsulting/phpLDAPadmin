...
class Template extends xmlTemplate
{
    ...
    protected function storeTemplate($xmldata)
    {
            ...
            switch ($xml_key) {
                    ...
                    if (isset($xmldata['template'][$xml_key]['objectclass'])) {
                        if (is_array($xmldata['template'][$xml_key]['objectclass'])) {
                            foreach ($xmldata['template'][$xml_key]['objectclass'] as $index => $details) {
                                ...
                                } elseif (!is_object($soc) && !$_SESSION[APPCONFIG]->getValue(
                                        'appearance',
                            ...
                            # XML files with only 1 objectClass dont have a numeric index.
                            $soc = $server->getSchemaObjectClass(
                                strtolower($xmldata['template'][$xml_key]['objectclass'])
                            );
                ...
                default:
                    ...
                    if (!in_array($xml_key, $allowed_arrays) && is_array($xml_value)) {
                        ...
                        error(
                            sprintf(
                                _('In the XML file (%s), [%s] is an array, it must be a string.'),
                                $this->filename,
                                $xml_key
                            ),
                            'error'
        ...
        foreach ($this->rdn as $key) {
            if ((is_null($attribute = $this->getAttribute($key))) && (in_array_ignore_case(
                    'extensibleobject',
    ...
    public function accept($makeVisible = false, $nocache = false)
    {
        ...
        if ($this->dn) {
            ...
            $counter = 1;
            foreach ($server->getDNAttrValues(
                $this->dn,
                null,
                LDAP_DEREF_NEVER,
                array_merge(array('*'), $server->getValue('server', 'custom_attrs')),
                $nocache
            ) as $attr => $values) {
                # We ignore DNs.
                if ($attr == 'dn') {
                ...
                foreach ($attribute->getValues() as $index => $value) {
                    if (in_array(
                        sprintf(
                            '%s=%s',
                            $attribute->getName(),
                            strtolower($attribute->getValue($index))
                        ),
                        $rdnarray
                    )) {
                        $attribute->setRDN($counter++);
            ...
            foreach ($server->getDNAttrValues(
                ...
                null,
                LDAP_DEREF_NEVER,
                array_merge(array('+'), $server->getValue('server', 'custom_sys_attrs'))
            ) as $attr => $values) {
                $attribute = $this->getAttribute($attr);
            ...
            if ($this->isType('default') && !count(
                    $this->getAttributes(true)
                ) && isset($_REQUEST['new_values']['objectclass'])
            ) {
                $attribute = $this->addAttribute(
                    'objectclass',
                    array('values' => $_REQUEST['new_values']['objectclass'])
                );
                $attribute->justModified();
                $this->rebuildTemplateAttrs();
                        ...
                        if (!is_uploaded_file($_FILES[$key]['tmp_name'][$attr][$index])) {
                            if (isset($_FILES[$key]['error'][$attr][$index])) {
                                switch ($_FILES[$key]['error'][$attr][$index]) {
                                    ...
                                    case 1:
                                        $msg = _(
                                            'The file you uploaded is too large. Please check php.ini, upload_max_size setting'
                                        );
                                        break;
                                    ...
                                    case 3:
                                        $msg = _(
                                            'The file you selected was only partially uploaded, likley due to a network error.'
                                        );
                                        break;
                ...
                if ($_FILES['single_item_value']['size'] === 0) {
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s %s',
                                _('The file you chose is either empty or does not exist.'),
                ...
                system_message(
                    array(
                        ...
                        'body' => sprintf(
                            ...
                            sprintf(
                                _(
                                    'RDN attribute sequence [%s] is already used by attribute [%s] and cant be used by attribute [%s] also.'
                                ),
                                $attribute->isRDN(),
                                $return[$attribute->isRDN()],
                                $attribute->getName()
                            )
                        ),
                        'type' => 'error'
            ...
            case 'autoFill':
                if (!preg_match('/;/', $arg)) {
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s (<b>%s</b>)',
                                _('There is only 1 argument, when there should be two'),
                ...
                foreach ($matchall[0] as $index => $null) {
                    ...
                    if (strstr($match_mod, 'T')) {
                        $attribute->js['autoFill'] .= sprintf(
                            "  %s = elem$elem_id.options[elem$elem_id.selectedIndex].text;\n",
                            $match_attr
                        );
                    } else {
                ...
                if ($soc->getMustAttrs()) {
                    foreach ($soc->getMustAttrs() as $index => $details) {
                        ...
                        if ((strcasecmp('objectClass', $objectclassattr) != 0) ||
                            ((strcasecmp('objectClass', $objectclassattr) == 0) && (!is_null(
                                    $this->getAttribute($objectclassattr)
                                )))
                        ) {
                    ...
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                ...
                                $this->getTitle(),
                                $attribute->getName(false),
                                _('removed from template as it is not defined by an ObjectClass')
                            ),
                            'type' => 'warn'
                        )
                    );
    ...
    public function getLDAPmodify($attrsOnly = false, $index = 0)
    {
        ...
        if ($this->getAttribute('objectclass')->hasBeenModified()) {
            ...
            foreach ($this->getAttribute('objectclass')->getRemovedValues() as $value) {
                ...
                foreach ($soc->getMustAttrs() as $value) {
                    ...
                    if ($attribute && (!in_array($value->getName(), $attr_to_keep)) && ($value->getName(
                            ) != 'objectclass')
        ...
        foreach ($this->getAttributes(true) as $attribute) {
            if ($attribute->hasBeenModified()
                && (count(array_diff($attribute->getValues(), $attribute->getOldValues())) || !count(
                        $attribute->getValues()
                    )
                    || $attribute->isForceDelete() || (count($attribute->getValues()) != count(
    ...
    public function getAvailAttrs()
    {
        ...
        if (in_array_ignore_case('extensibleobject', $this->getObjectClasses())) {
            foreach ($server->SchemaAttributes() as $sattr) {
                $attribute = $attribute_factory->newAttribute(
                    $sattr->getName(),
                    array('values' => array()),
                    $server->getIndex(),
                    null
