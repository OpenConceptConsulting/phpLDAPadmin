...
class PageRender extends Visitor
{
                ...
                foreach (get_request('post_value', 'REQUEST') as $attr => $values) {
                    ...
                    if (!$attribute) {
                        debug_dump_backtrace(
                            sprintf('There was a post_value for an attribute [%s], but it doesnt exist?', $attr),
                            1
                        );
                    }
        ...
        switch ($autovalue['function']) {
            ...
            case 'Join':
                ...
                foreach ($matchattrs as $joinattr) {
                    ...
                    if (!$attribute2) {
                        if (($pv = get_request(strtolower($joinattr), 'REQUEST')) && isset($pv[$attribute->getName(
                                )][$i])
                        ) {
                        ...
                        system_message(
                            array(
                                ...
                                'body' => sprintf(
                                    '%s (<b>%s [%s]</b>)',
                                    _('Function() variable expansion can only handle 1 value'),
                                    $attribute->getName(false),
                                    count($attribute->getValues())
                                ),
                                'type' => 'warn'
                            )
                        );
                ...
                if (count($args) != 2) {
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s (<b>%s</b>)',
                                _('PasswordEncrypt() only accepts two arguments'),
                                $autovalue['args']
                            ),
                            'type' => 'warn'
                        )
                    ...
                    if (count($matchall[1]) != 1) {
                        system_message(
                            array(
                                ...
                                'body' => sprintf(
                                    '%s (<b>%s</b>)',
                                    _('Unable to get the attribute value for PasswordEncrypt()'),
                                    count($matchall[1])
                                ),
                                'type' => 'warn'
                            )
    ...
    protected function getTemplateChoice()
    {
        ...
        if ($this->template_id === $template->getID()) {
            ...
            if (DEBUGTMP) {
                printf(
                    '<font size=-2>%s:<u>%s</u></font><br />',
                    __METHOD__,
                    'Choosing the DEFAULT template, no other template applicable'
                );
            }
            ...
            if ($_SESSION[APPCONFIG]->getValue('appearance', 'disable_default_template') AND $this->getMode(
                ) == 'creation'
            ) {
            ...
            # If there is only 1 defined template, and no default available, then that is our template.
                    ...
                    $templates->getTemplates($this->getMode(), $this->getModeContainer(), true)
                ) == 1) && !$this->haveDefaultTemplate()
        ) {
            if (DEBUGTMP) {
                printf(
                    '<font size=-2>%s:<u>%s</u></font><br />',
                    __METHOD__,
                    'AUTOMATIC choosing a template, only 1 template applicable'
                );
            }
    ...
    final protected function drawNameAttribute($attribute)
    {
        ...
        $href = sprintf(
            'cmd.php?cmd=schema&server_id=%s&view=attributes&viewvalue=%s',
            $this->getServerID(),
            $attribute->getName()
        );
        ...
        if (!$_SESSION[APPCONFIG]->getValue(
            ...
            printf(
                ...
                _('Click to view the schema definition for attribute type'),
                $attribute->getName(false),
                _($attribute->getFriendlyName())
            );
        } else {
            printf(
                '<acronym title="%s">%s</acronym>',
                _('This attribute is not defined in the LDAP schema'),
                _($attribute->getFriendlyName())
            );
        }
    ...
    protected function getNoteAliasAttribute($attribute)
    {
        ...
        if (strtolower($friendly_name) != $attribute->getName()) {
            return sprintf(
                '<acronym title="%s: \'%s\' %s \'%s\'">%s</acronym>',
                _('Note'),
                $friendly_name,
                _('is an alias for'),
                $attribute->getName(false),
    ...
    protected function getNoteRequiredAttribute($attribute)
    {
        ...
        if ($required_by) {
            return sprintf(
                '<acronym title="%s %s">%s</acronym>',
                _('Required attribute for objectClass(es)'),
                $required_by,
                _('required')
            );
        } else {
    ...
    final protected function drawHiddenValueAttribute($attribute, $i)
    {
        ...
        printf(
            '<input type="hidden" name="new_values[%s][%s]" id="new_values_%s_%s" value="%s" />',
    ...
    protected function drawFormReadOnlyValueAttribute($attribute, $i)
    {
        ...
        printf(
            '<input type="text" class="roval" name="new_values[%s][%s]" id="new_values_%s_%s" value="%s" readonly="readonly" />',
            htmlspecialchars($attribute->getName()),
            $i,
            htmlspecialchars($attribute->getName()),
            $i,
    ...
    protected function drawFormReadWriteValueAttribute($attribute, $i)
    {
        ...
        printf(
            '<input type="text" class="value" name="new_values[%s][%s]" id="new_values_%s_%s" value="%s" %s%s %s %s/>',
            htmlspecialchars($attribute->getName()),
            $i,
            htmlspecialchars($attribute->getName()),
            $i,
        ...
        if ($attribute->getVerify()) {
            printf(
                '<tr><td><input type="text" class="value" name="new_values_verify[%s][%s]" id="new_values_verify_%s_%s" value="" %s %s/>',
                htmlspecialchars($attribute->getName()),
                $i,
                htmlspecialchars($attribute->getName()),
                $i,
    ...
    final protected function drawHiddenValueBinaryAttribute($attribute, $i)
    {
        ...
        printf(
            '<input type="hidden" name="new_values[%s][%s]" value="%s" />',
            htmlspecialchars($attribute->getName()),
            $i,
            base64_encode($val)
        );
    ...
    protected function drawFormReadOnlyValueBinaryAttribute($attribute, $i)
    {
        ...
        $href = sprintf(
            'download_binary_attr.php?server_id=%s&dn=%s&attr=%s&index=%s',
            $this->getServerID(),
            rawurlencode($this->template->getDN()),
            $attribute->getName(),
            $i
        );

        printf(
            '<a href="%s"><img src="%s/save.png" alt="Save" /> %s</a>',
            htmlspecialchars($href),
            IMGDIR,
            _('download value')
        );
    ...
    protected function drawFormReadWriteValueBinaryAttribute($attribute, $i)
    {
        ...
        if ($attribute->getValue($i)) {
            ...
            if (!$attribute->isReadOnly() && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'delete_attr')) {
                printf(
                    '<a href="javascript:deleteAttribute(\'%s\',\'%s\',\'%s\');" style="color:red;"><img src="%s/trash.png" alt="Trash" /> %s</a>',
                    $attribute->getName(),
            ...
            printf(
                '<input type="file" class="value" name="new_values[%s][%s]" id="new_values_%s_%s" value="" %s%s %s %s/><br />',
                htmlspecialchars($attribute->getName()),
                $i,
                htmlspecialchars($attribute->getName()),
                $i,
    ...
    protected function drawFormReadWriteValueDateAttribute($attribute, $i)
    {
        ...
        printf(
            '<input type="text" class="value" id="new_values_%s_%s" name="new_values[%s][%s]" value="%s" %s%s %s %s/>&nbsp;',
            $attribute->getName(),
            $i,
            htmlspecialchars($attribute->getName()),
            $i,
    ...
    protected function drawFormReadWriteValueDnAttribute($attribute, $i)
    {
        ...
        printf(
            '<span style="white-space: nowrap;"><input type="text" class="value" name="%s" id="%s" value="%s" %s%s %s %s/>&nbsp;',
            $input_name,
            $id,
            htmlspecialchars($val),
            $attribute->needJS('focus') ? sprintf('onfocus="focus_%s(this);" ', $attribute->getName()) : '',
    ...
    protected function drawFormReadWriteValueGidAttribute($attribute, $i)
    {
        ...
        if ($this->template->getDN() && !in_array_ignore_case(
                'posixGroup',
                $this->getServer()->getDNAttrValue(
                    $this->template->getDN(),
                    'objectclass'
                )
            )
            ...
            $bases = array_unique(
                array_merge(array($server->getContainerTop($this->template->getDN())), $server->getBaseDN())
            );
            ...
            foreach ($bases as $base) {
                ...
                if (count($group) > 0) {
                    ...
                    $href = sprintf(
                        'cmd.php?cmd=template_engine&server_id=%s&dn=%s',
                        $this->getServerID(),
    ...
    protected function drawFormReadOnlyValueMultiLineAttribute($attribute, $i)
    {
        ...
        printf(
            '<textarea class="roval" rows="%s" cols="%s" name="new_values[%s][%s]" id="new_values_%s_%s" readonly="readonly">%s</textarea>',
    ...
    protected function drawFormReadWriteValueMultiLineAttribute($attribute, $i)
    {
        ...
        printf(
            '<textarea class="value" rows="%s" cols="%s" name="new_values[%s][%s]" id="new_values_%s_%s" %s%s>%s</textarea>',
            ($attribute->getRows() > 0) ? $attribute->getRows() : 5,
            ($attribute->getCols() > 0) ? $attribute->getCols() : 100,
            htmlspecialchars($attribute->getName()),
            $i,
    ...
    protected function drawFormValueObjectClassAttribute($attribute, $i)
    {
        ...
        if ($structural) {
            ...
            printf(
                ' <small>(<acronym title="%s">%s</acronym>)</small>',
    ...
    protected function getAutoPostPasswordAttribute($attribute, $i)
    {
        ...
        $attribute->setPostValue(
            array('function' => 'PasswordEncrypt', 'args' => sprintf('%%enc%%;%%%s%%', $attribute->getName()))
        );
    ...
    protected function drawFormReadOnlyValuePasswordAttribute($attribute, $i)
    {
        ...
        printf(
            '<input type="%s" class="roval" name="new_values[%s][%s]" id="new_values_%s_%s" value="%s" %s readonly="readonly" /><br />',
    ...
    protected function drawFormReadWriteValuePasswordAttribute($attribute, $i)
    {
        ...
        printf(
            '<input type="%s" class="value" name="new_values[%s][%s]" id="%s" value="%s" %s%s %s %s/>',
            ($obfuscate_password ? 'password' : 'text'),
            htmlspecialchars($attribute->getName()),
            $i,
            $id,
        ...
        if ($attribute->getVerify() && $obfuscate_password) {
            printf(
                '<input type="password" class="value" name="new_values_verify[%s][%s]" id="new_values_verify_%s_%s" value="" %s %s/>',
                htmlspecialchars($attribute->getName()),
                $i,
                htmlspecialchars($attribute->getName()),
                $i,
    ...
    protected function drawFormReadWriteValueSelectionAttribute($attribute, $i)
    {
        ...
        if ($attribute->isMultiple()) {
            ...
            if (($attribute->getSize() > 0) && ($attribute->getSize() < $attribute->getOptionCount())) {
                ...
                printf(
                    '<select name="new_values[%s][]" size="%s" multiple="multiple">',
                    htmlspecialchars($attribute->getName()),
                    $attribute->getSize()
                );
                ...
                foreach ($attribute->getSelection() as $value => $description) {
                    ...
                    printf(
                        '<option id="new_values_%s_%s" value="%s" onmouseDown="focus_%s(this);" onclick="blur_%s(this);" %s>%s</option>',
                        htmlspecialchars($attribute->getName()),
                        $j++,
                        $value,
                        htmlspecialchars($attribute->getName()),
                ...
                foreach ($vals as $val) {
                    if (!isset($selected[$val])) {
                        printf(
                            '<option id="new_values_%s_%s" value="%s" onmousedown="focus_%s(this);" onclick="blur_%s(this);" selected="selected">%s</option>',
                            htmlspecialchars($attribute->getName()),
                            $j++,
                            $val,
                            htmlspecialchars($attribute->getName()),
                ...
                printf(
                    '<tr><td colspan="2"><input type="hidden" id="new_values_%s_%s" name="new_values[%s][]" value="%s"/></td></tr>',
                    htmlspecialchars($attribute->getName()),
                    $j++,
                    htmlspecialchars($attribute->getName()),
                    ''
                ...
                foreach ($attribute->getSelection() as $value => $description) {
                    ...
                    printf(
                        '<tr><td><input type="checkbox" id="new_values_%s_%s" name="new_values[%s][]" value="%s" %s%s %s/></td><td><span style="white-space: nowrap;">&nbsp;%s</span></td></tr>',
                        htmlspecialchars($attribute->getName()),
                        $j++,
                        htmlspecialchars($attribute->getName()),
                        $value,
                ...
                foreach ($vals as $val) {
                    if (!isset($selected[$val])) {
                        printf(
                            '<tr><td><input type="checkbox" id="new_values_%s_%s" name="new_values[%s][]" value="%s" %s%s checked="checked"/></td><td><span style="white-space: nowrap;">&nbsp;%s</span></td></tr>',
                            htmlspecialchars($attribute->getName()),
                            $j++,
                            htmlspecialchars($attribute->getName()),
                            $val,
            ...
            if ($attribute->isRequired() && !count($attribute->getSelection())) {
                system_message(
                    array(
                        ...
                        'body' => sprintf(
                            'This template uses a selection list for attribute [<b>%s</b>], however the selection list is empty.<br />You may need to create some dependancy entries in your LDAP server so that this attribute renders with values. Alternatively, you may be able to define the appropriate selection values in the template file.',
                            $attribute->getName(false)
                        ),
                        'type' => 'warn'
                    )
            ...
            printf(
                '<select name="new_values[%s][]" id="new_values_%s_%s" %s%s>',
                htmlspecialchars($attribute->getName()),
                htmlspecialchars($attribute->getName()),
                $i,
                $attribute->needJS('focus') ? sprintf('onfocus="focus_%s(this);" ', $attribute->getName()) : '',
            ...
            foreach ($attribute->getSelection() as $value => $description) {
                printf(
                    ...
                    $value,
                    ((strcasecmp($value, $val) == 0) && $found = true) ? 'selected="selected"' : '',
                    $description
                );
