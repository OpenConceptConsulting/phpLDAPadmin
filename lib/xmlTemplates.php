        ...
        if ($this->templates = get_cached_item($server_id, $class['item'])) {
                ...
                if (!file_exists($template->getFileName())) {
                    ...
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s %s (%s)',
                                _('Template XML file has removed'),
                ...
                if (($template->getReadTime() < (time() - $class['cachetime']))
                    ...
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s %s (%s)',
                                _('Template XML file has changed and been reread'),
                                $template->getName(false),
                                $template->getType()
                            ),
                            'type' => 'info',
                    ...
                    $changed = true;
                    $this->templates[$index] = new $class['name'](
                        $this->server_id,
                        $template->getName(false),
                    ...
                    if (!in_array($filename, $this->getTemplateFiles())) {
                        ...
                        system_message(
                            array(
                                ...
                                'body' => sprintf(
                                    '%s %s (%s)',
                                    _('A new template XML file has been loaded'),
                                    $file,
                                    $type
                                ),
                                'type' => 'info',
            ...
            foreach ($class['types'] as $type) {
                ...
                while ($file = readdir($dh)) {
                    ...
                    # Store the template
                    $templatename = preg_replace('/.xml$/', '', $file);
                    $this->templates[$counter] = new $class['name'](
                        $this->server_id,
                        $templatename,
                        $filename,
                        $type,
    ...
    public function getTemplates($type = null, $container = null, $disabled = false)
    {
        ...
        if (is_array($this->templates)) {
            foreach ($this->templates as $details) {
                ...
                if (!is_null($container) && ($regexp = $template->getRegExp()) && (!@preg_match(
                        '/' . $regexp . '/i',
                        $container
                    ))
                ) {
        ...
        if (preg_match('/;/', $name)) {
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        '%s {%s} (%s)',
                        'PLA might not do what you expect...',
                        $name,
