...
class TemplateRender extends PageRender
{
    ...
    public function accept($norender = false)
    {
        ...
        if ($this->template_id AND $this->template_id != 'invalid') {
            ...
            $this->url_base = sprintf(
                'server_id=%s&dn=%s',
                $this->getServerID(),
                $this->template->getDNEncode()
            );
            $this->layout['hint'] = sprintf(
                '<td class="icon"><img src="%s/light.png" alt="%s" /></td><td colspan="3"><span class="hint">%%s</span></td>',
                IMGDIR,
                _('Hint')
            );
            ...
            foreach ($this->template->getAttributes(true) as $attribute) {
                ...
                if ($attribute->getAutoValue()) {
                    $this->get(
                        'Default',
                        $attribute,
                        $this->template->getContainer() ? $this->template->getContainer() :
                            $this->getServer()->getContainerPath($this->template->getDN()),
                        'autovalue'
                    );
                }
    ...
    protected function getDefaultAttribute($attribute, $container, $type)
    {
            ...
            default:
                system_message(
                    array(
                        ...
                        'body' => sprintf(
                            '%s (<b>%s</b>)',
                            _('A call was made to getDefaultAttribute() with an unkown context'),
                            $type
                        ),
                        'type' => 'warn'
                    )
        ...
        switch ($autovalue['function']) {
                ...
                if (count($args) && count($args) > 1) {
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s (<b>%s</b>)',
                                _('Function() only takes two arguments and more than two were specified'),
                                count($args)
                            ),
                            'type' => 'warn'
                        )
                ...
                if (function_exists($function)) {
                    ...
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s (<b>%s</b>)',
                                _('An attempt was made to call a function that doesnt exist'),
                                $function
                            ),
                            'type' => 'warn'
                        )
            ...
            case 'PickList':
                ...
                $picklistvalues = return_ldap_hash(
                    $container,
                    $args[1],
                    $args[2],
                    $attrs,
                ...
                if (isset($args[6])) {
                    ...
                    foreach ($fixedvalues as $fixedvalue) {
                        ...
                        $newvalue[trim($fixedvalue[0])] = array(
                            $args[2] => trim($fixedvalue[0]),
    ...
    protected function drawTemplateChoice()
    {
        ...
        foreach ($templates as $name => $details) {
            ...
            if ($isInValid) {
                ...
                if (isAjaxEnabled()) {
                    printf(
                        '<td><input type="radio" name="template" value="%s" id="%s" onclick="return ajDISPLAY(\'BODY\',\'%s&amp;template=%s\',\'%s\');" /></td>',
                        htmlspecialchars($details->getID()),
                        htmlspecialchars($details->getID()),
                        htmlspecialchars($href_parms),
                        $details->getID(),
                        str_replace('\'', '\\\'', _('Retrieving DN'))
                    );
                } else {
                    printf(
                        '<td><input type="radio" name="template" value="%s" id="%s" onclick="document.getElementById(\'template_choice_form\').submit()" /></td>',
                        htmlspecialchars($details->getID()),
                        htmlspecialchars($details->getID())
                    );
                }
            ...
            printf(
                '<td class="icon"><label for="%s"><img src="%s" alt="" /></label></td>',
                htmlspecialchars($details->getID()),
                $details->getIcon()
            );
            printf(
                '<td class="label"><label for="%s">',
                htmlspecialchars($details->getID())
            );

            if ($isInValid) {
                printf(
                    '<span id="%s" style="color: gray"><acronym title="%s">',
                    htmlspecialchars($details->getID()),
                    _($isInValid)
                );
            }
        ...
        if ($this->haveDefaultTemplate()) {
            ...
            if (isAjaxEnabled()) {
                printf(
                    '<td><input type="radio" name="template" value="none" id="none" onclick="return ajDISPLAY(\'BODY\',\'%s&amp;template=%s\',\'%s\');" /></td>',
                    htmlspecialchars($href_parms),
                    'none',
                    str_replace('\'', '\\\'', _('Retrieving DN'))
                );
        ...
        switch ($this->getMode()) {
            case 'creation':
                $subtitle = sprintf(
                    ...
                    _('Container'),
                    htmlspecialchars($this->container)
                );
                ...
                if ($this->template_id) {
                    ...
                    $subtitle .= sprintf(
                        '%s: <b>%s</b>',
                        _('Template'),
                        $this->template->getID() != 'none' ? $this->template->getTitle() : _('Default')
                    );
    ...
    private function getMenuItem($i)
    {
        ...
        switch ($i) {
            ...
            case 'entryexport':
                if ($_SESSION[APPCONFIG]->isCommandAvailable(
                        'script',
                        'export_form'
                    ) && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'export')
                ) {
            ...
            case 'entrycopy':
                if ($_SESSION[APPCONFIG]->isCommandAvailable(
                        'script',
                        'copy_form'
                    ) && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'copy') && !$this->template->isReadOnly()
                ) {
            ...
            case 'entrydelete':
                if ($_SESSION[APPCONFIG]->isCommandAvailable(
                        'script',
                        'delete_form'
                    ) && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'delete') && !$this->template->isReadOnly()
                ) {
            ...
            case 'entryrename':
                if ($_SESSION[APPCONFIG]->isCommandAvailable(
                        'script',
                        'rename_form'
                    ) && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'rename') && !$this->template->isReadOnly()
                ) {
            ...
            case 'msgdel':
                if ($_SESSION[APPCONFIG]->getValue('appearance', 'show_hints')
                    && $_SESSION[APPCONFIG]->isCommandAvailable(
                        'script',
                        'delete_form'
                    ) && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'delete') && !$this->template->isReadOnly()
                ) {
            ...
            case 'entrycompare':
                if ($_SESSION[APPCONFIG]->isCommandAvailable(
                        'script',
                        'compare_form'
                    ) && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'compare') && !$this->template->isReadOnly()
                ) {
            ...
            case 'childexport':
                ...
                if ($children_count === false) {
                    ...
                    if (!$children_count || !$more_children) {
                        ...
                        $all_children = $this->getServer()->getContainerContents(
                            ...
                            $children_count + 1,
                            '(objectClass=*)',
                            $_SESSION[APPCONFIG]->getValue('deref', 'view'),
                            null
                        );
                ...
                if ($children_count > 0 || $more_children) {
                    ...
                    } elseif ($i == 'childexport' && $_SESSION[APPCONFIG]->isCommandAvailable(
                            'script',
                            'export_form'
                        ) && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'export')
                    ) {
            ...
            case 'msgschema':
                if ($_SESSION[APPCONFIG]->getValue(
                        'appearance',
                        'show_hints'
                    ) && $_SESSION[APPCONFIG]->isCommandAvailable('script', 'schema')
                ) {
    ...
    protected function getDeleteAttributeMessage()
    {
        ...
        if ($_SESSION[APPCONFIG]->isCommandAvailable('script', 'delete_attr') && !$this->template->isReadOnly()) {
            return sprintf(
                $this->layout['hint'],
                _('Hint: To delete an attribute, empty the text field and click save.')
            );
        } else {
            return '';
        }
    ...
    protected function getModifiedAttributesMessage(&$modified_attributes)
    {
        ...
        return sprintf(
            ...
            (count($modified_attributes) == 1) ?
                sprintf(
                    _('An attribute (%s) was modified and is highlighted below.'),
                    implode('', $modified_attributes)
                ) :
                sprintf(
                    _('Some attributes (%s) were modified and are highlighted below.'),
                    implode(', ', $modified_attributes)
                )
        );
    }
    ...
    protected function getViewSchemaMessage()
    {
        ...
        return sprintf(
            $this->layout['hint'],
            _('Hint: To view the schema for an attribute, click the attribute name.')
        );
    }
    ...
    private function getMenuItemRefresh()
    {
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                $this->layout['actionajax'],
                IMGDIR,
                'refresh.png',
                _('Refresh'),
                htmlspecialchars($href),
                _('Refresh this entry'),
                htmlspecialchars($href),
                str_replace('\'', '\\\'', _('Reloading')),
                _('Refresh')
            );
        } else {
            return sprintf(
    ...
    protected function getMenuItemSwitchTemplate()
    {
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                ...
                _('Switch Template'),
                htmlspecialchars($href),
                _('Change to another template'),
                htmlspecialchars($href),
                str_replace('\'', '\\\'', _('Loading')),
                _('Switch Template')
            );
        } else {
            return sprintf(
    ...
    private function getMenuItemDelete()
    {
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                ...
                _('Trash'),
                htmlspecialchars($href),
                _('You will be prompted to confirm this decision'),
                htmlspecialchars($href),
                str_replace('\'', '\\\'', _('Loading')),
                _('Delete this entry')
            );
    ...
    protected function getMenuItemCompare()
    {
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                ...
                _('Compare'),
                htmlspecialchars($href),
                _('Compare this entry with another'),
                htmlspecialchars($href),
                str_replace('\'', '\\\'', _('Loading')),
                _('Compare with another entry')
            );
        } else {
            return sprintf(
                $this->layout['action'],
    ...
    protected function getMenuItemCreate()
    {
        ...
        $href = sprintf(
            'cmd=template_engine&server_id=%s&container=%s',
            $this->getServerID(),
            $this->template->getDNEncode()
        );
    ...
    protected function getMenuItemAddAttribute()
    {
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                ...
                _('Add'),
                htmlspecialchars($href),
                _('Add new attribute to this object'),
                htmlspecialchars($href),
                str_replace('\'', '\\\'', _('Add new attribute')),
                _('Add new attribute')
            );
        } else {
            return sprintf(
    ...
    protected function getMenuItemShowChildren($children_count)
    {
        ...
        $href = sprintf(
            'cmd=query_engine&server_id=%s&filter=%s&base=%s&scope=one&query=none&size_limit=0&search=true',
            $this->getServerID(),
            rawurlencode('objectClass=*'),
            $this->template->getDNEncode()
        );
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                ...
                _('Children'),
                htmlspecialchars($href),
                _('View the children of this object'),
                htmlspecialchars($href),
                str_replace('\'', '\\\'', _('Loading')),
    ...
    protected function getMenuItemExportSub()
    {
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                ...
                _('Save'),
                htmlspecialchars($href),
                _('Save a dump of this object and all of its children'),
                htmlspecialchars($href),
                str_replace('\'', '\\\'', _('Loading')),
                _('Export subtree')
            );
    ...
    protected function drawRDNChooser()
    {
        ...
        if (!count($this->template->getRDNAttrs())) {
            ...
            foreach ($this->template->getRDNAttrs() as $rdn) {
                printf(
                    '<input type="hidden" name="rdn_attribute[]" value="%s" id="rdn_attribute"/>',
    ...
    protected function drawContainerChooser($default_container)
    {
        ...
        if (get_request('create_base')) {
            printf(
                '%s<input type="hidden" name="container" size="40" value="%s" />',
                $default_container,
                htmlspecialchars($default_container)
            );
        } else {
    ...
    protected function drawObjectClassChooser()
    {
        ...
        foreach ($socs as $name => $oclass) {
            ...
            printf(
                ...
                htmlspecialchars($oclass->getName(false)),
                $oclass->getName(false)
            );
        }
    ...
    public function drawFormEnd()
    {
        ...
        if (!$_SESSION[APPCONFIG]->getValue('appearance', 'hide_debug_info') && get_request('debug', 'GET')) {
            echo "\n\n";
            printf(
                '<img src="%s/plus.png" alt="Plus" onclick="if (document.getElementById(\'DEBUGtemplate\').style.display == \'none\') { document.getElementById(\'DEBUGtemplate\').style.display = \'block\' } else { document.getElementById(\'DEBUGtemplate\').style.display = \'none\' };"/>',
                IMGDIR
            );
    ...
    public function drawFormSubmitButton()
    {
        ...
        if (!$this->template->isReadOnly()) // @todo cant use AJAX here, it affects file uploads.
        {
            printf(
                '<tr><td colspan="2" style="text-align: center;"><input type="submit" id="create_button" name="submit" value="%s" /></td></tr>',
                _('Update Object')
            );
        }
    }
    ...
    private function drawStepTitle($page)
    {
        ...
        if (DEBUGTMP || DEBUGTMPSUB) {
            printf('<font size=-2>* %s [templateNAME:%s]</font><br />', __METHOD__, $this->template->getName());
        }
    ...
    private function drawStepFormStart($page)
    {
        ...
        if (($this->template->isType('default') && $this->template->getContext(
                ) == 'create' && $page == 1) || $page < $this->pagelast
        ) {
    ...
    private function drawStepFormSubmitButton($page)
    {
        ...
        if ($page < $this->pagelast) {
            printf(
                '<td>&nbsp;</td><td><input type="submit" id="create_button" value="%s" /></td>',
                _('Proceed &gt;&gt;')
            );
        } else // @todo cant use AJAX here, it affects file uploads.
        {
            printf(
                '<td style="text-align: center;"><input type="submit" id="create_button" name="submit" value="%s" /></td>',
                _('Create Object')
            );
        }
        echo '</tr>';
...
function fillRec(id,value) {
    ...
    protected function drawTitleAttribute($attribute)
    {
        ...
        if (($this->template->getContext() == 'edit')
            && ($attribute->hasBeenModified() || in_array(
                    $attribute->getName(),
                    get_request('modified_attrs', 'REQUEST', false, array())
                ))
        ) {
    ...
    protected function drawDnValueIconAttribute($attribute, $val)
    {
        ...
        if (strlen($val) <= 0) {
            ...
            printf(
                '<a href="cmd.php?cmd=template_engine&amp;server_id=%s&amp;dn=%s" title="%s %s"><img src="%s/ldap-alias.png" alt="Go" /></a>&nbsp;',
                $this->getServerID(),
                rawurlencode($val),
                _('Go to'),
                $val,
            ...
            printf(
                '<a title="%s %s"><img src="%s/nogo.png" alt="Go" /></a>&nbsp;',
                _('DN not available'),
                $val,
                IMGDIR
            );
    ...
    protected function getMenuItemAttribute($attribute, $action)
    {
        ...
        switch ($action) {
            ...
            case 'modify':
                if (in_array(
                    $attribute->getName(),
                    arrayLower($_SESSION[APPCONFIG]->getValue('modify_member', 'groupattr'))
                )) {
    ...
    protected function getAddValueMenuItemAttribute($attribute)
    {
        ...
        $href_parm = htmlspecialchars(
            sprintf(
                'cmd=add_value_form&server_id=%s&dn=%s&attr=%s',
                $this->getServerID(),
                $this->template->getDNEncode(),
                rawurlencode($attribute->getName(false))
            )
        );

        if (isAjaxEnabled()) {
            return sprintf(
                '(<a href="cmd.php?%s" title="%s %s" onclick="return ajDISPLAY(\'ADDVALUE%s\',\'%s&amp;raw=1\',\'%s\',1);">%s</a>)',
                $href_parm,
                _('Add an additional value to attribute'),
                $attribute->getName(false),
                $attribute->getName(),
                $href_parm,
                str_replace('\'', '\\\'', _('Add Value to Attribute')),
                _('add value')
            );
        } else {
            return sprintf(
                '(<a href="cmd.php?%s" title="%s %s">%s</a>)',
                $href_parm,
                _('Add an additional value to attribute'),
                $attribute->getName(false),
                _('add value')
    ...
    protected function getAddValueMenuItemObjectClassAttribute($attribute)
    {
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                '(<a href="cmd.php?%s" title="%s %s" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'%s\');">%s</a>)',
                $href_parm,
                _('Add an additional value to attribute'),
                $attribute->getName(false),
                $href_parm,
    ...
    protected function getModifyMemberMenuItemAttribute($attribute)
    {
        ...
        $href = sprintf(
            'cmd=modify_member_form&server_id=%s&dn=%s&attr=%s',
            $this->getServerID(),
            $this->template->getDNEncode(),
            rawurlencode($attribute->getName())
        );
        ...
        if (isAjaxEnabled()) {
            return sprintf(
                '(<a href="cmd.php?%s" title="%s: %s" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'%s\');">%s</a>)',
                htmlspecialchars($href),
                _('Modify members for'),
                $this->template->getDN(),
                htmlspecialchars($href),
                str_replace('\'', '\\\'', _('Modify group membership')),
    ...
    protected function getRenameMenuItemAttribute($attribute)
    {
        ...
        $href = sprintf(
            'cmd.php?cmd=rename_form&server_id=%s&dn=%s&template=%s',
            $this->getServerID(),
            $this->template->getDNEncode(),
            $this->template->getID()
        );
    ...
    protected function drawHelperAttribute($attribute, $i)
    {
        ...
        if (is_array($id)) {
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        '%s (<b>%s</b>)',
                        _('Helper attribute has too many ID values, only the first one is used'),
                        count($id)
                    ),
                    'type' => 'warn'
                )
        ...
        if (is_array($display)) {
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        '%s (<b>%s</b>)',
                        _('Helper attribute has too many DISPLAY values, only the first one is used'),
                        count($display)
                    ),
                    'type' => 'warn'
                )
        ...
        if (is_array($vals) && count($vals) > 0) {
            ...
            printf(
                '<input type="text" name="%s[%s][%s]" id="%s_%s_%s" value="%s" size="4" />',
                $id,
                htmlspecialchars($attribute->getName()),
                $i,
                $id,
...
function deleteAttribute(attrName,friendlyName,i)
{
    ...
    protected function drawJavaScriptDateAttribute($attribute)
    {
        ...
        for ($i = 0; $i <= $attribute->getValueCount(); $i++) {
            printf(
                '<script type="text/javascript">defaults[\'new_values_%s_%s\'] = \'%s\';</script>',
                $attribute->getName(),
                $i,
                $config['format']
            );
            ...
            if (in_array_ignore_case(
                    $attribute->getName(),
                    array_keys($config['time'])
                ) && ($config['time'][$attribute->getName()])
            ) {
                printf(
                    '<script type="text/javascript">defaults[\'f_time_%s_%s\'] = \'%s\';</script>',
                    $attribute->getName(),
                    $i,
                    'true'
                );
    ...
    {
        printf(
            '<a href="javascript:dateSelector(\'%s_%s\');" title="%s"><img src="%s/calendar.png" alt="Calendar" class="chooser" id="f_trigger_%s_%s" style="cursor: pointer;" /></a>',
            $attribute->getName(),
            $i,
            _('Click to popup a dialog to select a date graphically'),
            IMGDIR,
            $attribute->getName(),
            $i
        );
    ...
    protected function drawIconObjectClassAttribute($attribute, $val)
    {
        ...
        if (strlen($val) > 0) {
            $href = sprintf(
                'cmd.php?cmd=schema&server_id=%s&view=objectclasses&viewvalue=%s',
                $this->getServerID(),
                $val
            );
            printf(
                '<a href="%s" title="%s"><img src="%s/info.png" alt="Info" /></a>&nbsp;',
                htmlspecialchars($href),
                _('View the schema description for this objectClass'),
                IMGDIR
            );
        }
    }
...
<script type="text/javascript">
    ...
    protected function drawCheckLinkPasswordAttribute($attribute, $component_id)
    {
        ...
        printf(
            '<small><a href="javascript:passwordComparePopup(\'%s\',\'%s\')">%s...</a></small><br />',
            $component_id,
            $attribute->getName(),
            _('Check password')
        );
    ...
    protected function drawJavascriptRandomPasswordAttribute($attribute)
    {
        ...
        printf(
            'alert("It seems another element was found, PLA hasnt been configured for this situation Component: "+component.value+" I:"+i);',
            $attribute->getName()
        );
        echo "\n";
        echo '}' . "\n";
    ...
    protected function drawDefaultHelperSambaPasswordAttribute($attribute, $i)
    {
        ...
        printf(
            '<input type="hidden" name="%s[%s][%s]" id="%s_%s_%s" value="%s" />',
            $id,
            htmlspecialchars($attribute->getName()),
            $i,
            $id,
