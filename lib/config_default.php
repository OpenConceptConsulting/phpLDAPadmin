    ...
    public function getValue($key, $index, $fatal = true)
    {
        ...
        if (!isset($config[$key])) {
            if ($fatal) {
                error(
                    sprintf(
                        'A call was made in [%s] to getValue requesting [%s] that isnt predefined.',
        ...
        if (!isset($config[$key][$index])) {
            if ($fatal) {
                error(
                    sprintf('Requesting an index [%s] in key [%s] that isnt predefined.', $index, $key),
                    'error',
                    null,
                    true
                );
        ...
        return isset($config[$key][$index]['value']) ? $config[$key][$index]['value'] :
            $config[$key][$index]['default'];
    }
            ...
            foreach ($this->custom as $masterkey => $masterdetails) {
                ...
                if (isset($this->default->$masterkey)) {
                    ...
                    if (!is_array($masterdetails)) {
                        error(
                            sprintf('Error in configuration file, [%s] should be an ARRAY.', $masterdetails),
                            'error',
                            null,
                            true
                        );
                    ...
                    foreach ($masterdetails as $key => $value) {
                        ...
                        if (!in_array($key, array_keys($this->default->$masterkey))) {
                            error(
                                sprintf(
                                    'Error in configuration file, [%s] has not been defined as a configurable variable.',
                                    $key
                                ),
                                'error',
                                null,
                        ...
                        if (is_array($this->default->{$masterkey}[$key]['default']) && !is_array($value)) {
                            error(
                                sprintf(
                                    'Error in configuration file, %s[\'%s\'] SHOULD be an array of values.',
                                    $masterkey,
                                    $key
                                ),
                                'error',
                        ...
                        if (!is_array($this->default->{$masterkey}[$key]['default']) && is_array($value)) {
                            error(
                                sprintf(
                                    'Error in configuration file, %s[\'%s\'] should NOT be an array of values.',
                                    $masterkey,
                                    $key
                                ),
                                'error',
                    ...
                    error(
                        sprintf(
                            'Error in configuration file, [%s] has not been defined as a MASTER configurable variable.',
                            $masterkey
                        ),
                        'error',
                        null,
