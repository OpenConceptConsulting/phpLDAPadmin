...
class AJAXTree extends HTMLTree
{
     ...
     *
     * @param string $dn The Base DN to draw
     ...
     */
    protected function draw_item($dn, $level, $first_child = true, $last_child = true)
    {
        ...
        # Links
        $parms['openclose'] = htmlspecialchars(
            sprintf('server_id=%s&dn=%s&code=%s%s', $this->getServerID(), $entry->getDNEncode(), $code, $new_code[$nb])
        );
        $parms['edit'] = htmlspecialchars(
            sprintf('cmd=template_engine&server_id=%s&dn=%s', $this->getServerID(), $entry->getDNEncode())
        );
        ...
        if (!$child_count) {
            printf(
                '<img id="jt%snode" src="%s/%s" alt="--" class="imgs" style="border: 0; vertical-align:text-top;" />',
                $node_id,
                IMGDIR,
                $imgs['tree'][$nb]
            );
            ...
            printf(
                '<a href="#" onclick="return opencloseTreeNode(\'%s\',\'%s\',\'%s\');">',
                $node_id,
                $parms['openclose'],
                IMGDIR
            );
        ...
        printf(
            '<a href="%s" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'%s\');" title="%s" >',
            $href,
            $parms['edit'],
            _('Retrieving DN'),
            htmlspecialchars($entry->getDN())
        );
        printf(
            '<span class="dnicon"><img id="jt%sfolder" src="%s/%s" alt="->" class="imgs" style="border: 0; vertical-align:text-top;" /></span>',
            $node_id,
            IMGDIR,
            $entry->getIcon($server)
        );
        echo '</a>';

        echo '&nbsp;';
        printf(
            '<a href="%s" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'%s\');" title="%s" class="phplm">',
            $href,
            $parms['edit'],
            _('Retrieving DN'),
            htmlspecialchars($entry->getDN())
        ...
        printf(
            '<div id="jt%sson" style="display: %s" class="treemenudiv">',
            $node_id,
            ($entry->isOpened() ? 'block' : 'none')
        );
        if ($entry->isOpened()) {
     ...
     *
     * @param string $code a string of 0 and 1 ; $code == "000101" will return "   | |"
     * @return string
     */
    protected function get_indentation($code)
    {
        ...
        for ($i = 0; $i < strlen($code); $i++) {
            switch ($code[$i]) {
                case '0':
                    $indent .= sprintf(
                        '<img src="%s/tree_space.png" alt="  " class="imgs" style="border: 0; vertical-align:text-top;" />',
                        IMGDIR
                    );
                    break;
                ...
                case '1':
                    $indent .= sprintf(
                        '<img src="%s/tree_vertline.png" alt="| " class="imgs" style="border: 0; vertical-align:text-top;" />',
                        IMGDIR
                    );
                    break;
            }
    ...
    private function create_before_child($entry, $level)
    {
        ...
        if (!$server->isReadOnly() && !$entry->isLeaf() && (count($entry->getChildren()) > 10) && $this->getServer(
            )->isShowCreateEnabled()
    ...
    private function draw_create_new_entry($entry, $level, $img)
    {
        ...
        $output .= sprintf(
            '<img src="%s" alt="--" class="imgs" style="border: 0; vertical-align:text-top;" />',
            $img
        );
        $output .= sprintf('<a href="%s" title="%s">', htmlspecialchars($href), $entry->getDN());
        $output .= sprintf(
            '<img src="%s/create.png" alt="->" class="imgs" style="border: 0; vertical-align:text-top;" />',
            IMGDIR
        );
        $output .= '</a>';
        $output .= '&nbsp;';
        ...
        if (isAjaxEnabled()) {
            $output .= sprintf(
                '<a href="cmd.php?%s" title="%s" class="phplm" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'%s\');">',
            ...
            $output .= sprintf(
                '<a href="cmd.php?%s" title="%s" class="phplm">',
                htmlspecialchars($href),
                _('Create new entry here')
            );
        }
     ...
     *
     * @return string[] List of open nodes
     */
