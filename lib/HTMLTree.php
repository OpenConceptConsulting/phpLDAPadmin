...
class HTMLTree extends Tree
{
     ...
     *
     * @param boolean $onlytree Only display the tree, or include the server name and menu items
     */
        ...
        if ($server->isLoggedIn(null)) {
            if (!$onlytree) {
                ...
                if ($server->isReadOnly()) {
                    printf(
                        '<tr><td class="spacer"></td><td class="logged_in" colspan="%s">(%s)</td></tr>',
                        $this->getDepth() + 3 - 1,
                        _('read only')
                    );
                } else {
                ...
                printf(
                    '<tr><td>&nbsp;</td><td><div style="overflow: auto; %s%s" id="ajSID_%s_nodes">',
                    $_SESSION[APPCONFIG]->getValue('appearance', 'tree_width') ?
                        sprintf('width: %spx; ', $_SESSION[APPCONFIG]->getValue('appearance', 'tree_width')) : '',
                    $_SESSION[APPCONFIG]->getValue('appearance', 'tree_height') ?
                        sprintf('height: %spx; ', $_SESSION[APPCONFIG]->getValue('appearance', 'tree_height')) : '',
                    $server->getIndex()
                );
            }
            ...
            if (!count($this->getBaseEntries())) {
                ...
                printf(
                    '<tr><td class="spacer"></td><td class="spacer"></td><td colspan="%s"><small>%s<br />%s<br /><b>%s</b></small></td></tr>',
                    $this->getDepth() + 3 - 2,
            ...
            foreach ($this->getBaseEntries() as $base) {
                if (!$base->isInLDAP()) {
                    ...
                    printf(
                        '<tr><td class="spacer"></td><td class="spacer"></td><td><img src="%s/unknown.png" alt="" /></td><td colspan="%s">%s</td></tr>',
                        IMGDIR,
                        $this->getDepth() + 3 - 3,
                        pretty_print_dn($base->getDN())
                    );

                    $this->javascript .= sprintf(
                        '<form id="create_base_form_%s_%s" method="post" action="cmd.php">',
                        $server->getIndex(),
                        $javascript_id
                    );
                    $this->javascript .= '<div>';
                    ...
                    $this->javascript .= sprintf(
                        '<input type="hidden" name="server_id" value="%s" />',
                        $server->getIndex()
                    );
                    $this->javascript .= sprintf(
                        '<input type="hidden" name="container" value="%s" />',
                        htmlspecialchars($server->getContainer($base->getDN()))
                    );
                    $this->javascript .= sprintf(
                        '<input type="hidden" name="rdn" value="%s" />',
                        get_rdn($base->getDN())
                    );
                    $this->javascript .= sprintf('<input type="hidden" name="rdn_attribute[]" value="%s" />', $rdn[0]);
                    $this->javascript .= sprintf(
                        '<input type="hidden" name="new_values[%s][]" value="%s" />',
                        $rdn[0],
                        $rdn[1]
                    );
                    ...
                    if (preg_match('/,/', $base->getDN())) {
                        printf(
                            '<tr><td class="spacer"></td><td class="spacer"></td><td class="spacer"></td><td colspan="%s"><small>%s</small></td></tr>',
                            $this->getDepth() + 3 - 3,
                            _('This base cannot be created with PLA.')
                        );
                    } else {
                        printf(
                            '<tr><td class="spacer"></td><td class="spacer"></td><td class="spacer"></td><td colspan="%s"><small>%s <a href="javascript:document.getElementById(\'create_base_form_%s_%s\').submit()">%s</a></small></td></tr>',
                            $this->getDepth() + 3 - 3,
                            _('This base entry does not exist.'),
                            $server->getIndex(),
                            $javascript_id,
                            _('Create it?')
                        );
            ...
            switch ($server->getAuthType()) {
                ...
                default:
                    die(sprintf(
                        'Error: %s hasnt been configured for auth_type %s',
                        __METHOD__,
                        $server->getAuthType()
                    ));
            }
    ...
    protected function draw_server_name()
    {
        ...
        if (!is_null($server->inactivityTime())) {
            $m = sprintf(
                _('Inactivity will log you off at %s'),
    ...
    protected function get_menu_item($item)
    {
        ...
        switch ($item) {
            ...
            case 'import':
                if (!$_SESSION[APPCONFIG]->isCommandAvailable(
                        'script',
                        'import_form'
                    ) || !$_SESSION[APPCONFIG]->isCommandAvailable('script', 'import') || $server->isReadOnly()
                ) {
                    return '';
                }
        ...
        $href_parms = htmlspecialchars(
            sprintf(
                'cmd=%s&server_id=%s%s',
                $menu['cmd'],
                $server->getIndex(),
                isset($menu['href']) ? $menu['href'] : ''
            )
        );

        if (isAjaxEnabled()) {
            return sprintf(
                '<a href="cmd.php?%s" onclick="return ajDISPLAY(\'%s\',\'%s\',\'%s\');" title="%s %s"><img src="%s/%s" alt="%s" /><br />%s</a>',
                $href_parms,
                $menu['div'],
                $href_parms,
                $menu['ajax'],
            ...
            return sprintf(
                '<a href="cmd.php?%s" title="%s %s"><img src="%s/%s" alt="%s" /><br />%s</a>',
                $href_parms,
                $menu['title'],
                $server->getName(),
                IMGDIR,
    ...
    protected function get_logout_menu_item()
    {
        ...
        if (!$_SESSION[APPCONFIG]->isCommandAvailable('script', 'logout') || in_array(
                $server->getAuthType(),
                array(
                    'config',
                    'http',
            ...
            return sprintf(
                '<a href="%s" title="%s"><img src="%s/%s" alt="%s" /><br />%s</a>',
                htmlspecialchars($href),
                _('Logout of this server'),
                IMGDIR,
                'logout-big.png',
                _('logout'),
                _('logout')
     ...
     */
    protected function draw_item($dn, $level)
    {
        if (DEBUG_ENABLED && (($fargs = func_get_args()) || $fargs = 'NOARGS')) {
            debug_log('Entered (%%)', 33, 0, __FILE__, __LINE__, __METHOD__, $fargs);
        }
        ...
        # If the entry doesn't exist, we'll add it.
        if (!$entry) {
            $this->addEntry($dn);
            $entry = $this->getEntry($dn);
        }

        # If the entry doesn't exist in the server, then return here with an empty string.
        if (!$entry) {
            return;
        }
        ...
        $href['expand'] = htmlspecialchars(
            sprintf('cmd.php?cmd=expand&server_id=%s&dn=%s', $server->getIndex(), $dnENCODE)
        );
        $href['collapse'] = htmlspecialchars(
            sprintf('cmd.php?cmd=collapse&server_id=%s&dn=%s', $server->getIndex(), $dnENCODE)
        );
        $href['edit'] = htmlspecialchars(
            sprintf('cmd.php?cmd=template_engine&server_id=%s&dn=%s', $server->getIndex(), $dnENCODE)
        );
        ...
        if ($entry->isOpened()) {
            if (!$child_count && !$this->getServer()->isShowCreateEnabled()) {
                ...
                printf(
                    '<td class="expander"><a href="%s"><img src="%s/minus.png" alt="-" /></a></td>',
                    $href['collapse'],
                    IMGDIR
                );
            }
            ...
            if (($child_count !== false) && (!$child_count) && (!$this->getServer()->isShowCreateEnabled())) {
                ...
                printf(
                    '<td class="expander"><a href="%s"><img src="%s/plus.png" alt="+" /></a></td>',
                    $href['expand'],
                    IMGDIR
                );
            }
        ...
        printf(
            '<td class="icon"><a href="%s" id="node_%s_%s"><img src="%s/%s" alt="img" /></a></td>',
            $href['edit'],
            $server->getIndex(),
            preg_replace('/=/', '_', base64_encode($dn)),
            IMGDIR,
            $entry->getIcon()
        );

        printf(
            '<td class="phplm" colspan="%s" style="width: 100%%;"><span style="white-space: nowrap;">',
            $this->getDepth() + 3 - $level
        );
        ...
        if (DEBUG_ENABLED) {
            debug_log('Leaving (%s,%s)', 33, 0, __FILE__, __LINE__, __METHOD__, $dn, $level);
        }
    }
    ...
    protected function draw_create_link($rdn, $level, $encoded_dn)
    {
        ...
        $href = htmlspecialchars(
            sprintf('cmd.php?cmd=template_engine&server_id=%s&container=%s', $this->getServerID(), $encoded_dn)
        );
        ...
        printf(
            '<td class="link" colspan="%s"><a href="%s" title="%s %s">%s</a></td>',
            $this->getDepth() + 3 - $level,
            $href,
            _('Create a new entry in'),
            $rdn,
            _('Create new entry here')
        );
        echo '</tr>';
    }
    ...
    protected function draw_login_link()
    {
        ...
        $href_parm = htmlspecialchars(
            sprintf('cmd=%s&server_id=%s', get_custom_file($server->getIndex(), 'login_form', ''), $server->getIndex())
        );
        ...
        if (isAjaxEnabled()) {
            printf(
                '<td class="icon"><a href="cmd.php?%s" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'%s\');" title="%s %s"><img src="%s/%s" alt="%s" /></a></td>',
                $href_parm,
                $href_parm,
                _('Loading Login'),
                _('Login to'),
            ...
            printf(
                '<td class="logged_in" colspan="%s"><a href="cmd.php?%s" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'%s\');" title="%s %s">%s</a></td>',
                $this->getDepth() + 3 - 2,
                $href_parm,
                $href_parm,
                _('Loading Login'),
            ...
            printf(
                '<td class="icon"><a href="cmd.php?%s"><img src="%s/%s" alt="%s" /></a></td>',
                $href_parm,
                IMGDIR,
                'login.png',
                _('login')
            ...
            printf(
                '<td class="logged_in" colspan="%s"><a href="cmd.php?%s">%s...</a></td>',
                $this->getDepth() + 3 - 2,
                $href_parm,
                _('Login')
            );
