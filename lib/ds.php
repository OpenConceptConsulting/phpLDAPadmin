...
abstract class DS
{
    ...
    public function getValue($key, $setting, $fatal = true)
    {
        ...
        } elseif (isset($this->default->{$key}[$setting]) && array_key_exists(
                'default',
                $this->default->{$key}[$setting]
            )
        ) {
        ...
        if (isset($this->custom->{$key}[$setting])) {
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        'A call has been made to reset a configuration value (%s,%s,%s)',
                        $key,
                        $setting,
                        $value
                    ),
    ...
    public function getAuthType()
    {
        ...
        switch ($this->getValue('login', 'auth_type')) {
            ...
            default:
                die(sprintf(
                    'Error: <b>%s</b> hasnt been configured for auth_type <b>%s</b>',
                    __METHOD__,
    ...
    public function getLogin($method = null)
    {
        ...
        switch ($this->getAuthType()) {
            case 'cookie':
                if (!isset($_COOKIE[$method . '-USER'])) # If our bind_id is set, we'll pass that back for logins.
                {
                    return (!is_null($this->getValue('login', 'bind_id')) && $method == 'login') ?
                        $this->getValue('login', 'bind_id') : null;
                } else {
            ...
            case 'session':
            case 'sasl':
                if (!isset($_SESSION['USER'][$this->index][$method]['name'])) # If our bind_id is set, we'll pass that back for logins.
                {
                    return (!is_null($this->getValue('login', 'bind_id')) && $method == 'login') ?
                        $this->getValue('login', 'bind_id') : null;
                } else {
    ...
    protected function getPassword($method = null)
    {
        ...
        switch ($this->getAuthType()) {
            case 'cookie':
                if (!isset($_COOKIE[$method . '-PASS'])) # If our bind_id is set, we'll pass that back for logins.
                {
                    return (!is_null($this->getValue('login', 'bind_pass')) && $method == 'login') ?
                        $this->getValue('login', 'bind_pass') : null;
                } else {
            ...
            case 'session':
            case 'sasl':
                if (!isset($_SESSION['USER'][$this->index][$method]['pass'])) # If our bind_pass is set, we'll pass that back for logins.
                {
                    return (!is_null($this->getValue('login', 'bind_pass')) && $method == 'login') ?
                        $this->getValue('login', 'bind_pass') : null;
                } else {
    ...
    public function isLoggedIn($method = null)
    {
        ...
        switch ($this->getAuthType()) {
            case 'config':
                if (!$CACHE[$this->index][$method] = $this->login(
            ...
            case 'proxy':
                $CACHE[$this->index][$method] = $this->login(
                    $this->getValue('login', 'bind_id'),
                    $this->getValue('login', 'bind_pass'),
                    $method
                );
    ...
    public function isReadOnly()
    {
        ...
        if (!trim($this->getLogin(null)) && $_SESSION[APPCONFIG]->getValue(
                'appearance',
