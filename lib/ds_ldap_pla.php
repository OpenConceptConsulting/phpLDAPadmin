...
class ldap_pla extends ldap
{
    ...
    function isMultiLineAttr($attr_name, $val = null)
    {
        ...
        if (!$return) {
            ...
            if ($sattr) {
                ...
                if ($syntax_oid) {
                    foreach ($_SESSION[APPCONFIG]->getValue(
                        'appearance',
                        'multi_line_syntax_oids'
                    ) as $multi_line_syntax_oid) {
    ...
    public function add($dn, $entry_array, $method = null)
    {
        ...
        if (run_hook(
            'pre_entry_create',
            array('server_id' => $this->index, 'method' => $method, 'dn' => $dn, 'attrs' => $entry_array)
        )) {
    ...
    public function modify($dn, $attrs, $method = null)
    {
                ...
                if (!run_hook(
                    ...
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s (<b>%s</b>)',
                                _('Hook pre_attr_add prevented attribute from being added'),
                                $attr
                            ),
                            'type' => 'warn'
                        )
                ...
                if (!run_hook(
                    ...
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s (<b>%s</b>)',
                                _('Hook pre_attr_modify prevented attribute from being modified'),
                                $attr
                            ),
                            'type' => 'warn'
                        )
                ...
                if (!run_hook(
                    ...
                    system_message(
                        array(
                            ...
                            'body' => sprintf(
                                '%s (<b>%s</b>)',
                                _('Hook pre_attr_delete prevented attribute from being deleted'),
                                $attr
                            ),
                            'type' => 'warn'
                        )
        ...
        if (run_hook(
            'pre_entry_modify',
            array('server_id' => $this->index, 'method' => $method, 'dn' => $dn, 'attrs' => $attrs)
        )) {
    ...
    public function checkUniqueAttrs($dn, $attrs)
    {
        ...
        if (!$checkattrs = array_intersect(
            arrayLower($this->getValue('unique', 'attrs')),
            array_keys(array_change_key_case($attrs))
        )
        ) {
            return true;
        }
            ...
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        '%s (<b>%s</b>)',
                        _(
                            'Unable to connect to LDAP server with the unique login/password, please check your configuration.'
                        ),
                        $this->getName()
                    ),
                    'type' => 'warn'
        ...
        foreach ($this->getBaseDN() as $base) {
            ...
            # If we have a match.
            if (count($results)) {
                foreach ($results as $values) # If one of the attributes is owned to somebody else, then we may as well die here.
                {
                    if ($values['dn'] != $dn) {
                        $href = sprintf(
                            'cmd.php?cmd=query_engine&server_id=%s&filter=%s&scope=sub&query=none&format=list&search=true',
                            $this->index,
                            $query['filter']
                        );
