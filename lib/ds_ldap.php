...
*/
    ...
    protected function connect($method, $debug = false, $new = false)
    {
        ...
        $bind['id'] = is_null($this->getLogin($method)) && $method != 'anon' ? $this->getLogin('user') :
            $this->getLogin($method);
        $bind['pass'] = is_null($this->getPassword($method)) && $method != 'anon' ? $this->getPassword('user') :
        ...
        if (isset($CACHE[$this->index]) && !$new) {
            foreach ($CACHE[$this->index] as $cachedmethod => $resource) {
                if (($this->getLogin($cachedmethod) == $bind['id']) && ($this->getPassword(
                            $cachedmethod
                        ) == $bind['pass'])
                ) {
        ...
        if (DEBUG_ENABLED) {
            debug_log(
                'Creating NEW connection [%s] for index [%s]',
                16,
                0,
                __FILE__,
                __LINE__,
        ...
        if (DEBUG_ENABLED) {
            debug_log(
                ...
                __METHOD__,
                $resource,
                $this->getValue('server', 'host'),
                $this->getValue('server', 'port')
            );
        }
            ...
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        '<b>%s</b>: %s (%s) for <b>%s</b>',
                        _('Error'),
                        $this->getErrorMessage($method),
                        $this->getErrorNum($method),
                        $method
                    ),
                    'type' => 'error'
                )
    ...
    public function query($query, $method, $index = null, $debug = false)
    {
        ...
        if (!isset($query['attrs'])) {
            $query['attrs'] = array();
        } else # Re-index the attrs, PHP throws an error if the keys are not sequential from 0.
        {
        ...
        if (DEBUG_ENABLED) {
            debug_log(
                'Search scope [%s] base [%s] filter [%s] attrs [%s] COMPLETE (%s).',
                16,
                0,
                __FILE__,
                __LINE__,
    ...
    private function isTLSEnabled()
    {
        ...
        if ($this->getValue('server', 'tls') && !function_exists('ldap_start_tls')) {
            error(
                _(
                    'TLS has been enabled in your config, but your PHP install does not support TLS. TLS will be disabled.'
                ),
                'warn'
            );
            ...
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        '<b>%s</b>: %s',
                        _('Error'),
                        _('Could not start TLS. Please check your LDAP server configuration.')
                    ),
                    'type' => 'error'
                )
            );
    ...
    private function isSASLEnabled()
    {
        ...
        if (!function_exists('ldap_sasl_bind')) {
            error(
                _(
                    'SASL has been enabled in your config, but your PHP install does not support SASL. SASL will be disabled.'
                ),
                'warn'
            );
            ...
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        ...
                        _('has not been implemented yet')
                    ),
                    'type' => 'error'
                )
            );
    ...
    private function startProxy($resource, $method)
    {
        ...
        if (!(isset($rootdse['supportedcontrol']) && in_array(
            ...
            system_message(
                array(
                    ...
                    'body' => sprintf(
                        '<b>%s</b>: %s',
                        _('Error'),
                        _('Your LDAP server doesnt seem to support this control')
                    ),
                    'type' => 'error'
                )
            );
                ...
                system_message(
                    array(
                        ...
                        'body' => sprintf(
                            '<b>%s</b>: %s (%s)',
                            _('Error'),
                            _('Search for DN returned the incorrect number of results'),
                            count($search)
                        ),
                        'type' => 'error'
                    )
    ...
    public function getBaseDN($method = null)
    {
        ...
        if (count($this->getValue('server', 'base'))) {
            if (DEBUG_ENABLED) {
                debug_log(
                    'Return BaseDN from Config [%s]',
                    17,
                    0,
                    __FILE__,
                    __LINE__,
                    __METHOD__,
                    implode('|', $this->getValue('server', 'base'))
                );
            }
            ...
            if (isset($results['namingcontexts'])) {
                if (DEBUG_ENABLED) {
                    debug_log(
                        ...
                        __LINE__,
                        __METHOD__,
                        implode('|', $results['namingcontexts'])
                    );
                }
     ...
     */
    public function getContainerContents(
        $dn,
        $method = null,
        $size_limit = 0,
        $filter = '(objectClass=*)',
    ...
    private function getRawSchema($method, $schema_to_fetch, $dn = '')
    {
        ...
        if ($schema_dn) {
            ...
            foreach (array('(objectClass=*)', '(objectClass=subschema)') as $schema_filter) {
                if (DEBUG_ENABLED) {
                    debug_log(
                        'Looking for schema with Filter (%s)',
                        24,
                        0,
                        __FILE__,
                        __LINE__,
                ...
                if (is_array($schema_entries) && isset($schema_entries['count']) && $schema_entries['count']) {
                    if (DEBUG_ENABLED) {
                        debug_log(
                            'Found schema with (DN:%s) (FILTER:%s) (ATTR:%s)',
                            24,
                            0,
                            __FILE__,
                            __LINE__,
                ...
                if (DEBUG_ENABLED) {
                    debug_log(
                        'Didnt find schema with filter (%s)',
                        24,
                        0,
                        __FILE__,
                        __LINE__,
        ...
        if (is_null($schema_search)) {
            if (DEBUG_ENABLED) {
                debug_log(
                    'Attempting work-arounds for "broken" LDAP servers...',
                    24,
                    0,
                    __FILE__,
                    __LINE__,
            ...
            foreach ($this->getBaseDN() as $base) {
                $ldap['W2K3 AD'][expand_dn_with_base(
                    $base,
        ...
        if (is_null($schema_search)) {
            ...
            if (!is_null($schema_search)) {
                ...
                if ($schema_entries) {
                    if (DEBUG_ENABLED) {
                        debug_log(
                            'Found schema with filter of (%s) and attribute filter (%s)',
                            24,
                            0,
                            __FILE__,
                            __LINE__,
                    ...
                    if (DEBUG_ENABLED) {
                        debug_log(
                            'Didnt find schema with filter (%s) and attribute filter (%s)',
                            24,
                            0,
                            __FILE__,
                            __LINE__,
        ...
        if (is_null($schema_search)) {
            ...
            foreach (array('base', 'one') as $ldap_scope) {
                if (DEBUG_ENABLED) {
                    debug_log(
                        'Attempting to find schema with scope (%s), filter (objectClass=*) and a blank base.',
                        24,
                        0,
                        __FILE__,
                        __LINE__,
            ...
            if (in_array($schema_to_fetch, $schema_error_message_array)) {
                system_message(
                    array(
                        'title' => sprintf(
                            '%s (%s)',
                            _('Our attempts to find your SCHEMA have failed'),
                            $schema_to_fetch
                        ),
        ...
        if (!$schema_entries) {
            ...
            if (DEBUG_ENABLED) {
                debug_log(
                    'Returning false since ldap_get_entries() returned false.',
                    25,
                    0,
                    __FILE__,
                    __LINE__,
        ...
        if ($olc_schema_found) {
            ...
            foreach ($schema_entries as $entry) {
                if (isset($entry[$olc_schema])) {
                    ...
                    foreach ($entry[$olc_schema] as $schema_definition) /* Schema definitions in child nodes prefix the schema entries with "{n}"
        ...
        if (!isset($schema_entries[0][$schema_to_fetch])) {
            if (in_array($schema_to_fetch, $schema_error_message_array)) {
                error(
                    sprintf(
                        'Our attempts to find your SCHEMA for "%s" have return UNEXPECTED results.<br /><br /><small>(We expected a "%s" in the $schema array but it wasnt there.)</small><br /><br />%s<br /><br />Dump of $schema_search:<hr /><pre><small>%s</small></pre>',
                        $schema_to_fetch,
                        gettype($schema_search),
                        $schema_error_message,
                        serialize($schema_entries)
                    ),
                    'error',
                    'index.php'
                );
                ...
                if (DEBUG_ENABLED) {
                    debug_log(
                        'Returning because (%s) isnt in the schema array. (%s)',
                        25,
                        0,
                        __FILE__,
                        __LINE__,
                ...
                if (trim($sup_attr_name)) {
                    ...
                    while ($i++ < 100/** 100 == INFINITY ;) */) {
                        ...
                        if (!isset($attrs[strtolower($sup_attr_name)])) {
                            error(
                                sprintf(
                                    'Schema error: attributeType "%s" inherits from "%s", but attributeType "%s" does not exist.',
                                    $attr->getName(),
                                    $sup_attr_name,
                                    $sup_attr_name
                                ),
     ...
     * @see getDNAttrValue
     */
    public function getDNAttrValues(
        $dn,
        $method = null,
        $deref = LDAP_DEREF_NEVER,
        $attrs = array('*', '+'),
        ...
        foreach ($this->getValue('login', 'allowed_dns') as $login_allowed_dn) {
            ...
            if (preg_match('/^\([&|]\(/', $login_allowed_dn)) {
                ...
                foreach ($this->getBaseDN() as $base_dn) {
                    ...
                    if (DEBUG_ENABLED) {
                        debug_log(
                            'Search, Filter [%s], BaseDN [%s] Results [%s]',
                            16,
                            0,
                            __FILE__,
                            __LINE__,
