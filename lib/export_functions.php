                ...
                foreach ($dndetails as $key => $attr) {
                    ...
                    foreach ($attr as $value) {
                        $output .= sprintf(
                            '%s<value>%s</value>%s',
                            $indent['val'],
                            ($binary_mode ? base64_encode($value) : htmlspecialchars($value)),
                            $this->br
                        );
                    }
    ...
    function export()
    {
        ...
        foreach ($this->results as $base => $results) {
            foreach ($results as $dndetails) {
                ...
                $output .= sprintf(
                    'UID:%s%s',
                    isset($dndetails['entryUUID']) ? $dndetails['entryUUID'] : $dndetails['dn'],
                    $this->br
                );
