...
class ObjectClass extends SchemaItem
{
    ...
    public function __construct($class, $server)
    {
        ...
        if (DEBUG_ENABLED) {
            debug_log(
                'Returning () - NAME (%s), DESCRIPTION (%s), MUST (%s), MAY (%s), FORCE MAY (%s)',
                9,
                0,
                __FILE__,
                __LINE__,
        ...
        for ($i = 0; $i < count($strings); $i++) {
            ...
            switch ($strings[$i]) {
                ...
                case 'SINGLE-VALUE':
                    ...
                    if (DEBUG_ENABLED) {
                        debug_log(
                            'Case SINGLE-VALUE returned (%s)',
                            8,
                            0,
                            __FILE__,
                            __LINE__,
                            __METHOD__,
                            $this->is_single_value
                        );
                    }
                    break;
                ...
                case 'NO-USER-MODIFICATION':
                    ...
                    if (DEBUG_ENABLED) {
                        debug_log(
                            'Case NO-USER-MODIFICATION returned (%s)',
                            8,
                            0,
                            __FILE__,
                            __LINE__,
                            __METHOD__,
                            $this->is_no_user_modification
                        );
                    }
                    break;
