...
if (isset($app['server']) && !is_null($request['container'])) {
    ...
    $request['children'] = $app['server']->getContainerContents(
        ...
        0,
        '(objectClass=*)',
        $_SESSION[APPCONFIG]->getValue('deref', 'tree')
    );
    ...
    foreach ($app['server']->getBaseDN() as $base) {
        if (DEBUG_ENABLED) {
            debug_log(
                'Comparing BaseDN [%s] with container [%s]',
                64,
                0,
                __FILE__,
                __LINE__,
        ...
        if (!pla_compare_dns($request['container'], $base)) {
            ...
            $href['up'] = sprintf(
                'entry_chooser.php?form=%s&element=%s&rdn=%s',
                $request['form'],
                $request['element'],
                rawurlencode($request['rdn'])
            );
            break;
            ...
            $href['up'] = sprintf(
                'entry_chooser.php?form=%s&element=%s&rdn=%s&server_id=%s&container=%s',
                $request['form'],
                $request['element'],
                $request['rdn'],
                $app['server']->getIndex(),
    ...
    if (!count($request['children'])) {
        ...
        foreach ($request['children'] as $dn) {
            $href['return'] = sprintf(
                "javascript:returnDN('%s%s')",
                ($request['rdn'] ? sprintf('%s,', $request['rdn']) : ''),
                str_replace('\\', '\\\\', $dn)
            );
            $href['expand'] = sprintf(
                'entry_chooser.php?server_id=%s&form=%s&element=%s&rdn=%s&container=%s',
                $app['server']->getIndex(),
                $request['form'],
                $request['element'],
                $request['rdn'],
            ...
            echo '<td class="blank">&nbsp;</td>';
            printf(
                '<td class="icon"><a href="%s"><img src="%s/plus.png" alt="Plus" /></a></td>',
                $href['expand'],
                IMGDIR
            );
...
# Draw the root of the selection tree (ie, list all the servers)
    ...
    foreach ($_SESSION[APPCONFIG]->getServerList() as $index => $server) {
        if ($server->isLoggedIn(null)) {
            printf(
                '<tr><td class="heading" colspan="3">%s:</td><td class="heading">%s</td></tr>',
                _('Server'),
                $server->getName()
            );
            foreach ($server->getBaseDN() as $dn) {
                if (!$dn) {
                    printf(
                        '<tr><td class="blank">&nbsp;</td><td colspan="3">(%s)</td></tr>',
                        _('Could not determine base DN')
                    );
                    ...
                    echo '<td class="blank">&nbsp;</td>';
                    printf(
                        '<td colspan="2" class="icon"><a href="%s"><img src="%s/plus.png" alt="Plus" /></a></td>',
                        $href['expand'],
                        IMGDIR
                    );
