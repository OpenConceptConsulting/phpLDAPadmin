...
if (count($request['update'])) {
    if (get_request('confirm', 'REQUEST')) {
        ...
        echo '<tr><td>';
        printf(
            _('There will be %s updates done with this mass update'),
            sprintf('<b>%s</b>', count($request['update']))
        );
        echo '</td></tr>';
        echo '</table>';
        ...
        foreach ($request['update'] as $index => $page) {
            ...
            printf(
                ...
                IMGDIR,
                get_icon($app['server']->getIndex(), $template->getDN())
            );

            printf(
                '<td colspan="3"><a href="cmd.php?cmd=template_engine&amp;server_id=%s&amp;dn=%s">%s</a></td>',
                $app['server']->getIndex(),
                rawurlencode(dn_unescape($template->getDN())),
                htmlspecialchars(get_rdn($template->getDN()))
            );
            echo '</tr>';

            printf(
                '<tr class="list_item"><td class="blank">&nbsp;</td><td class="heading">dn</td><td class="value" style="width: 45%%;">%s</td><td class="value" style="width: 45%%;"><b>%s</b></td></tr>',
                htmlspecialchars(dn_unescape($template->getDN())),
                _('Old Value')
            );
            ...
            foreach ($template->getLDAPmodify(true, $index) as $attribute) {
                ...
                if (!$attribute->getValueCount() || $attribute->isForceDelete()) {
                    ...
                    printf(
                        '<input type="hidden" name="mass_values[%s][%s][%s]" value="%s" />',
                        $index,
                        $attribute->getName(),
                        0,
                        ''
                ...
                foreach ($attribute->getOldValues() as $key => $value) {
                    # For multiple values, we'll highlight the changed ones
                    if ((count($attribute->getOldValues()) > 5) && in_array(
                            $value,
