...
if (count($request['template']->getLDAPmodify(true))) {
    ...
    printf(
        '<tr class="heading"><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>',
        _('Attribute'),
        _('Old Value'),
        _('New Value'),
        _('Skip')
    ...
    foreach ($request['template']->getLDAPmodify(true) as $attribute) {
        ...
        foreach ($attribute->getOldValues() as $key => $value) {
            ...
            if ($x = ((count($attribute->getOldValues()) > 5) && count($attribute->getValues()) && in_array(
                    $value,
                    $dv
                ))
            ) {
        ...
        foreach ($attribute->getValues() as $key => $value) {
            # For multiple values, we'll highlight the changed ones
            if ($x = ((count($attribute->getValues()) > 5) && count($attribute->getOldValues()) && in_array(
                    $value,
                    $dv
                ))
            ) {
        ...
        if ($attribute->isForceDelete() || (in_array(
                $attribute->getName(),
                $mustattrs
            )) && $request['template']->getAttribute('objectclass')->justModified()
        ) {
            ...
            if (count($request['template']->getForceDeleteAttrs()) > 0) {
                ...
                foreach ($request['template']->getForceDeleteAttrs() as $ad_name) {
                    ...
                    if (!in_array($ad_name->getName(), getMustAttrs($attribute->getOldValues()))) {
                        $input_onclick .= sprintf(
                            "document.getElementById('skip_array_%s').disabled = false;",
                            $ad_name->getName()
                        );
                    }

                    $input_onclick .= sprintf(
                        "document.getElementById('skip_array_%s').checked = true;",
                        $ad_name->getName()
                    );
                    $input_onclick .= "\n";
                }
                ...
                foreach ($request['template']->getForceDeleteAttrs() as $ad_name) {
                    $input_onclick .= sprintf(
                        "document.getElementById('skip_array_%s').checked = false;",
                        $ad_name->getName()
                    );
                    $input_onclick .= sprintf(
                        "document.getElementById('skip_array_%s').disabled = true;",
                        $ad_name->getName()
                    );
                    $input_onclick .= "\n";
                }
        ...
        printf(
            '<td><input name="skip_array[%s]" id="skip_array_%s" type="checkbox" %s %s/></td>',
    ...
    printf(
        '<input type="submit" name="cancel" value="%s" %s/>',
        _('Cancel'),
        (isAjaxEnabled() ?
            sprintf(
                'onclick="return ajDISPLAY(\'BODY\',\'cmd=template_engine&dn=%s\',\'%s\');"',
    ...
    if (count($request['template']->getForceDeleteAttrs()) > 0) {
        ...
        printf(
            ...
            implode('</b>, <b>', $request['template']->getAttribute('objectclass')->getRemovedValues())
        );
        echo '</tr><tr>';
