        ...
        if ((in_array($key, array('cache'))) && is_array($request['dumpvar'][$key])) {
            foreach (array_keys($request['dumpvar'][$key]) as $server) {
                foreach (array_keys($request['dumpvar'][$key][$server]) as $x) {
                    ...
                    printf(
                        '<li><span id="%s"><a href="javascript:get(\'%s\',\'%s\');">%s</a></span><div id="%sloading" style="display: none" ></div></li>',
                        $key . $index,
                        $key,
                        $index,
                        $key . '.' . $index,
            ...
            printf(
                '<li><span id="%s"><a href="javascript:get(\'%s\',\'\');">%s</a></span><div id="%sloading" style="display: none" ></div></li>',
                $key,
                $key,
                $key,
                $key
