...
$schema_error_str = sprintf(
    '%s <b>%s</b>.<br /><br /></div>%s<ul><li>%s</li><li>%s</li><li>%s</li><li>%s</li></ul>',
    _('Could not retrieve schema from'),
    $app['server']->getName(),
    _('This could happen for several reasons, the most probable of which are:'),
    _('The server does not fully support the LDAP protocol.'),
    _('Your version of PHP does not correctly perform the query.'),
    _('phpLDAPadmin doesn\'t know how to fetch the schema for your server.'),
...
foreach ($entry['schema_types'] as $item => $value) {
    ...
    if ($entry['view'] == $item) {
        ...
        if (isAjaxEnabled()) {
            printf(
                '<a href="cmd.php?%s" onclick="return ajDISPLAY(\'BODY\',\'%s\',\'Loading %s\');" title="Loading %s">%s</a>',
                htmlspecialchars($entry['href'][$item]),
                htmlspecialchars($entry['href'][$item]),
                $value,
                $value,
                $value
...
switch ($entry['view']) {
    ...
    case 'attributes':
        ...
        foreach ($sattrs as $name => $attr) {
            printf(
                ...
                $name,
                $name == $entry['value'] ? 'selected="selected" ' : '',
                $attr->getName(false)
            );
        }
        echo '</select>';
        ...
        foreach ($sattrs as $attr) {
            if (isAjaxEnabled() || (is_null($entry['value']) || !trim(
                        $entry['value']
                    ) || $entry['value'] == $attr->getName())
            ) {
                ...
                if (isAjaxEnabled() && $entry['value']) {
                    printf(
                        '<div id="at%s" style="display: %s">',
                        $attr->getName(),
                        strcasecmp($entry['value'], $attr->getName()) ? 'none' : 'block'
                    );
                } else {
                ...
                printf(
                    '<tr class="heading"><td colspan="2"><a name="%s">%s</a></td></tr>',
                    $attr->getName(),
                    $attr->getName(false)
                );
                    ...
                    switch ($item) {
                        ...
                        case 'inherits':
                            ...
                            if (is_null($attr->getSupAttribute())) {
                                ...
                                $href = htmlspecialchars(
                                    sprintf(
                                        '%s&viewvalue=%s',
                                        $entry['href']['attributes'],
                                        strtolower($attr->getSupAttribute())
                                    )
                                );
                                if (isAjaxEnabled()) {
                                    printf(
                                        '<a href="cmd.php?%s" onclick="return ajSHOWSCHEMA(\'attributes\',\'at\',\'%s\');">%s</a>',
                                        $href,
                                        strtolower($attr->getSupAttribute()),
                                        $attr->getSupAttribute()
                                    );
                                } else {
                        ...
                        case 'equality':
                            ...
                            if (is_null($attr->getEquality())) {
                                ...
                                $href = htmlspecialchars(
                                    sprintf('%s&viewvalue=%s', $entry['href']['matching_rules'], $attr->getEquality())
                                );
                                if (isAjaxEnabled()) {
                                    printf(
                                        '<a href="cmd.php?%s" onclick="return ajJUMP(\'%s\',\'%s\',\'%s\');">%s</a>',
                                        $href,
                                        $href,
                                        _('Matching Rules'),
                                        $attr->getEquality(),
                        ...
                        case 'syntax':
                            ...
                            if (is_null($attr->getType())) {
                                ...
                                $href = htmlspecialchars(
                                    sprintf('%s&highlight_oid=%s', $entry['href']['syntaxes'], $attr->getSyntaxOID())
                                );
                                if (isAjaxEnabled()) {
                                    printf(
                                        '<a href="cmd.php?%s" onclick="return ajJUMP(\'%s\',\'%s\',\'%s\');">%s (%s)</a>',
                                        $href,
                                        $href,
                                        _('Syntaxes'),
                                        '',
                        ...
                        case 'usage':
                            printf(
                                '<td>%s</td>',
                                $attr->getUsage() ? $attr->getUsage() : '(' . _('not specified') . ')'
                            );
                            break;
                        ...
                        case 'maximum_length':
                            ...
                            if (is_null($attr->getMaxLength())) {
                                ...
                                printf(
                                    '%s %s',
                                    number_format($attr->getMaxLength()),
                        ...
                        case 'aliases':
                            ...
                            if (count($attr->getAliases()) == 0) {
                                ...
                                foreach ($attr->getAliases() as $alias) {
                                    $href = htmlspecialchars(
                                        sprintf('%s&viewvalue=%s', $entry['href']['attributes'], strtolower($alias))
                                    );
                                    if (isAjaxEnabled()) {
                                        printf(
                                            '<a href="cmd.php?%s" onclick="return ajSHOWSCHEMA(\'attributes\',\'at\',\'%s\');">%s</a>',
                            ...
                            if (count($attr->getUsedInObjectClasses()) == 0) {
                                ...
                                foreach ($attr->getUsedInObjectClasses() as $objectclass) {
                                    $href = htmlspecialchars(
                                        sprintf(
                                            '%s&viewvalue=%s',
                                            $entry['href']['objectclasses'],
                                            strtolower($objectclass)
                                        )
                                    );
                                    if (isAjaxEnabled()) {
                                        printf(
    ...
    case 'matching_rules':
        ...
        echo '<table class="result_table" width="100%" border="0">';
        printf(
            '<tr class="heading"><td>%s</td><td>%s</td><td>%s</td></tr>',
            _('Matching Rule OID'),
            _('Name'),
            _('Used by Attributes')
        );
        ...
        foreach ($schema_matching_rules as $rule) {
            ...
            if (isAjaxEnabled() || (is_null($entry['value']) || !trim(
                ...
                if (isAjaxEnabled() && $entry['value']) {
                    printf(
                        '<tr class="%s" id="mr%s" style="display: %s">',
                        $counter % 2 ? 'odd' : 'even',
                        $rule->getName(),
                ...
                if (count($rule->getUsedByAttrs()) == 0) {
                    ...
                    if (isAjaxEnabled()) {
                        printf(
                            '<input type="button" value="%s" onclick="return ajJUMP(\'cmd=schema&amp;view=attributes&amp;server_id=%s\',\'%s\',\'%s\',\'vv\');"/>',
                            _('Go'),
                            $app['server']->getIndex(),
                            _('Attributes'),
                            $rule->getName()
    ...
    case 'objectclasses':
        ...
        foreach ($socs as $name => $oclass) {
            if (isAjaxEnabled() || (is_null($entry['value']) || !trim(
                ...
                if (isAjaxEnabled() && $entry['value']) {
                    printf(
                        '<div id="oc%s" style="display: %s">',
                        $oclass->getName(),
                        strcasecmp($entry['value'], $oclass->getName()) ? 'none' : ''
                    );
                } else {
                ...
                echo '<table class="result_table" width="100%" border="0">';
                printf(
                    '<tr class="heading"><td colspan="4"><a name="%s">%s</a></td></tr>',
                    $name,
                    $oclass->getName(false)
                );
                ...
                if ($oclass->getDescription()) {
                    printf(
                        '<tr class="odd"><td colspan="4">%s: <b>%s</b></td></tr>',
                        _('Description'),
                        $oclass->getDescription()
                    );
                }
                ...
                if (count($oclass->getSupClasses()) == 0) {
                    ...
                    foreach ($oclass->getSupClasses() as $i => $object_class) {
                        $href = htmlspecialchars(
                            sprintf('%s&viewvalue=%s', $entry['href']['objectclasses'], strtolower($object_class))
                        );
                        if (isAjaxEnabled()) {
                            printf(
                                '<a href="cmd.php?%s" onclick="return ajSHOWSCHEMA(\'objectclasses\',\'oc\',\'%s\');">%s</a>',
                                $href,
                                strtolower($object_class),
                                $object_class
                            );
                        } else {
                            printf(
                                '<a href="cmd.php?%s&viewvalue=%s" title="%s">%s</a>',
                                $href,
                                $object_class,
                ...
                if (strcasecmp($oclass->getName(), 'top') == 0) {
                    ...
                    if (isAjaxEnabled()) {
                        printf(
                            '<a href="cmd.php?%s" onclick="return ajSHOWSCHEMA(\'objectclasses\',\'oc\',\'\');">all</a>',
                            $href
                        );
                    } else {
                    ...
                    foreach ($oclass->getChildObjectClasses() as $i => $object_class) {
                        ...
                        if (isAjaxEnabled()) {
                            printf(
                                '<a href="cmd.php?%s" title="%s" onclick="return ajSHOWSCHEMA(\'objectclasses\',\'oc\',\'%s\');">%s</a>',
                                $href,
                                _('Jump to this objectClass definition'),
                                strtolower($object_class),
                                $object_class
                ...
                printf(
                    '<tr class="even"><td class="blank" rowspan="2" style="width: 5%%;">&nbsp;</td><td style="width: 45%%;"><b>%s</b></td><td style="width: 45%%;"><b>%s</b></td><td class="blank" rowspan="2" style="width: 5%%;">&nbsp;</td></tr>',
                ...
                if ($attrs = $oclass->getMustAttrs(true)) {
                    ...
                    foreach ($attrs as $attr) {
                        echo '<li>';
                        $href = htmlspecialchars(
                            sprintf('%s&viewvalue=%s', $entry['href']['attributes'], $attr->getName())
                        );
                        if (isAjaxEnabled()) {
                            printf(
                                '<a href="cmd.php?%s" onclick="return ajJUMP(\'%s\',\'%s\',\'%s\');">%s</a>',
                ...
                if ($attrs = $oclass->getMayAttrs(true)) {
                    ...
                    foreach ($attrs as $attr) {
                        ...
                        if ($oclass->isForceMay($attr->getName())) {
                            ...
                            printf(
                                '<small>%s</small>',
                                _('This attribute has been forced as a MAY attribute by the configuration')
                            );
                        }
                        echo '</li>';
                    }
