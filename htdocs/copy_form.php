...
printf(
    ...
    _('The full DN of the new entry to be created when copying the source entry'),
    _('Destination DN')
);
echo '<td>';
...
printf(
    ...
    _('Destination Server'),
    server_select_list($app['server']->getIndex(), true, 'server_id_dst')
);
echo "\n";
...
if (count($request['children']) > 0) {
    ...
    printf(
        '<td><acronym title="%s">%s</acronym>:</td>',
        _('When performing a recursive copy, only copy those entries which match this filter'),
        _('Filter')
    );
    ...
    printf(
        '<tr><td>%s</td><td><input type="checkbox" name="remove" value="yes"/></td></tr>',
        _('Delete after copy (move):')
    );
}
echo "\n";
...
if ($_SESSION[APPCONFIG]->getValue('appearance', 'show_hints')) {
    printf(
        '<div style="text-align: center;"><small><img src="%s/light.png" alt="Light" /><span class="hint">%s</span></small></div>',
        IMGDIR,
        _('Hint: Copying between different servers only works if there are no schema violations')
    );
}
