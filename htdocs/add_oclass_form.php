...
foreach ($ldap['attrs']['must'] as $attr) {
    if (is_null($request['template']->getAttribute($attr))) {
        array_push(
            $ldap['attrs']['need'],
            $attribute_factory->newAttribute($attr, array('values' => array()), $app['server']->getIndex())
        );
    }
}
...
if (count($ldap['attrs']['need']) > 0) {
    ...
    printf(
        '<div style="text-align: center;"><br /><input type="submit" value="%s" /></div>',
        _('Add ObjectClass and Attributes')
    );
    echo '</form>';
    echo '</div>';
...
# There are no other required attributes, so we just need to add the objectclass to the DN.
    ...
    if ($result) {
        $href = sprintf(
            'cmd.php?cmd=template_engine&server_id=%s&dn=%s&modified_attrs[]=objectclass',
            $app['server']->getIndex(),
