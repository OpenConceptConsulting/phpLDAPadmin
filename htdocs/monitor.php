...
if (!isset($attrs['monitorcontext']) || !count($results)) {
    system_message(
        array(
            ...
            'body' => sprintf(
                '%s: <b>%s</b>',
                _('Could not obtain the monitor context for this server'),
                $app['server']->getName()
            ),
            'type' => 'warn'
        ),
...
foreach (array(
    ...
    printf(
        '<tr class="list_item"><td class="heading" rowspan="2"><acronym title="%s">%s</acronym></td></tr>',
        $description,
        $dn
    );
    ...
    foreach ($results[$dn]['monitoredinfo'] as $index => $backend) {
        ...
        if (isset($results[$key]['seealso'])) {
            $seealso = is_array($results[$key]['seealso']) ? $results[$key]['seealso'] :
                array($results[$key]['seealso']);

            foreach ($seealso as $db) {
                if (isset($results[$db]['namingcontexts'])) {
                    printf(
                        '<acronym title="%s">%s</acronym><br/>',
...
printf(
    '<tr class="list_item"><td class="heading" rowspan="2"><acronym title="%s">%s</acronym></td></tr>',
    $results['cn=Connections,cn=Monitor']['description'][0],
    _('LDAP Connections')
);
printf('<tr class="list_item"><td class="value">');
echo '<table class="result"><tr><td>';
...
printf(
    '<tr class="highlight"><td class="20%%">%s</td><td class="value" style="width: 80%%;">%s</td></tr>',
    _('Total Connections'),
    $results['cn=Total,cn=Connections,cn=Monitor']['monitorcounter'][0]
);
printf(
    '<tr class="highlight"><td class="20%%">%s</td><td class="value" style="width: 80%%;">%s</td></tr>',
    _('Current Connections'),
    $results['cn=Current,cn=Connections,cn=Monitor']['monitorcounter'][0]
);
...
foreach ($results as $key => $value) {
    if (preg_match('/^cn=Connection.*,cn=Connections,cn=Monitor$/', $key)) {
        ...
        foreach (array(
            ...
            printf(
                '<td class="title" style="width: 35%%;">%s</td><td style="width: 65%%;">%s</td>',
                $metric,
                isset($results[$key][$metric]) ? $results[$key][$metric][0] : '&nbsp;'
            );
            echo '</tr>';
        }
...
foreach (array(
    ...
    if (isset($results[$dn]['monitoropinitiated'])) {
        printf(
            '<tr class="highlight"><td style="width: 20%%;">%s</td><td class="value" style="width: 80%%;">%s</td></tr>',
            'monitorOpInitiated',
            $results[$dn]['monitoropinitiated'][0]
        );
    }
    if (isset($results[$dn]['monitoropcompleted'])) {
        printf(
            '<tr class="highlight"><td style="width: 20%%;">%s</td><td class="value" style="width: 80%%;">%s</td></tr>',
            'monitorOpCompleted',
            $results[$dn]['monitoropcompleted'][0]
        );
    }
