...
printf(
    '<h3 class="subtitle">%s: <b>%s</b> &nbsp;&nbsp;&nbsp; %s: <b>%s</b></h3>',
    _('Server'),
    $app['server']->getName(),
...
if (count($request['children'])) {
    ...
    $search['href'] = htmlspecialchars(
        sprintf(
            'cmd.php?cmd=query_engine&server_id=%s&filter=%s&base=%s&scope=sub&query=none&format=list',
            $app['server']->getIndex(),
            rawurlencode('objectClass=*'),
            rawurlencode($request['dn'])
        )
    );
    ...
    printf(
        ...
        sprintf(
            _(
                'phpLDAPadmin can recursively delete this entry and all %s of its children. See below for a list of all the entries that this action will delete. Do you want to do this?'
            ),
    ...
    printf(
        '<tr><td colspan="2"><small>%s</small></td></tr>',
        _(
    ...
    printf(
        ...
        (isAjaxEnabled() ?
            sprintf(
                'onclick="return ajSUBMIT(\'BODY\',document.getElementById(\'delete_form\'),\'%s\');"',
                _('Deleting Object(s)')
            ) : '')
    );
    echo '</form>';
    ...
    printf(
        ...
        (isAjaxEnabled() ?
            sprintf(
                'onclick="return ajDISPLAY(\'BODY\',\'cmd=template_engine&server_id=%s&dn=%s\',\'%s\');"',
                $app['server']->getIndex(),
    ...
    $i = 0;
    printf(
        '<select size="%s" multiple disabled style="background:white; color:black;width:500px" >',
        min(10, count($request['search']))
    );
    ...
    printf(
        '<tr><td style="width: 10%%;">%s:</td><td colspan="3" style="width: 75%%;"><b>%s</b></td></tr>',
        _('Server'),
        $app['server']->getName()
    );
    printf(
        '<tr><td style="width: 10%%;"><acronym title="%s">%s</acronym></td><td colspan="3" style="width: 75%%;"><b>%s</b></td></tr>',
        _('Distinguished Name'),
        _('DN'),
        $request['dn']
    );
