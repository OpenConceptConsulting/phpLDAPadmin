...
foreach ($request['template']->getAttributes() as $attribute) {
    ...
    if ($attribute->isRequired() && !count($attribute->getValues()) && !$app['server']->isIgnoredAttr(
            $attr->getName()
        )
    ) {
        error(
            sprintf(
                _('You left the value blank for required attribute (%s).'),
...
if (count($request['template']->getLDAPadd(true))) {
    ...
    $counter = 0;
    printf(
        '<tr class="%s"><td colspan="3" style="text-align: center;"><b>%s</b></td></tr>',
        $counter % 2 ? 'even' : 'odd',
        htmlspecialchars($request['template']->getDN())
    );
    ...
    printf(
        ...
        (isAjaxEnabled() ?
            sprintf(
                'onclick="return ajSUBMIT(\'BODY\',document.getElementById(\'create_form\'),\'%s\');"',
                _('Updating Object')
            ) : '')
    );
    ...
    printf(
        ...
        (isAjaxEnabled() ?
            sprintf(
                'onclick="return ajDISPLAY(\'BODY\',\'cmd=template_engine&server_id=%s&container=%s\',\'%s\');"',
                $app['server']->getIndex(),
                $request['template']->getContainer(),
                _('Retrieving DN')
            ) : '')
