...
if ($ldap['DST']->dnExists($request['dnDST'])) {
    error(
        sprintf(_('The destination entry (%s) already exists.'), pretty_print_dn($request['dnDST'])),
        'error',
        'index.php'
    );
}
...
if (!$ldap['DST']->dnExists($ldap['DST']->getContainer($request['dnDST']))) {
    error(
        sprintf(
            _('The destination container (%s) does not exist.'),
            pretty_print_dn($ldap['DST']->getContainer($request['dnDST']))
        ),
        'error',
        'index.php'
    );
...
if ($request['recursive']) {
    ...
    if ($_SESSION[APPCONFIG]->getValue('confirm', 'copy')) {
        $request['pageSRC'] = new TemplateRender(
            $ldap['SRC']->getIndex(),
            get_request('template', 'REQUEST', false, null)
        );
        $request['pageSRC']->setDN($request['dnSRC']);
        $request['pageSRC']->accept(true);

        $request['pageDST'] = new TemplateRender(
            $ldap['DST']->getIndex(),
            get_request('template', 'REQUEST', false, 'none')
        );
...
if ($copy_result) {
    $redirect_url = sprintf(
        'cmd.php?cmd=template_engine&server_id=%s&dn=%s&refresh=SID_%s_nodes&noheader=1',
        $ldap['DST']->getIndex(),
        rawurlencode($request['dnDST']),
        $ldap['DST']->getIndex()
    );
...
function r_copy_dn($serverSRC, $serverDST, $snapshottree, $dnSRC, $dnDST, $remove)
{
    ...
    if (is_array($children) && count($children)) {
        ...
        array_push(
            $copy_message,
            sprintf('%s %s: <b>%s</b> %s', _('Copy successful'), _('DN'), $dnDST, _('has been created.'))
        );
...
function copy_dn($serverSRC, $serverDST, $dnSRC, $dnDST, $remove)
{
    ...
    if ($remove) {
        return $serverDST->rename(
            $request['templateSRC']->getDN(),
            $request['templateDST']->getRDN(),
            $serverDST->getContainer($dnDST),
            true
        );
    } else {
