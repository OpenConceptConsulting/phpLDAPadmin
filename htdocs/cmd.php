...
if (DEBUG_ENABLED) {
    debug_log(
        'Ready to render page for command [%s,%s].',
        128,
        0,
        __FILE__,
        __LINE__,
...
if (trim($www['cmd'])) {
    ...
    if (!$_SESSION[APPCONFIG]->isCommandAvailable('script', $www['cmd'])) {
        system_message(
            array(
                'title' => _('Command disabled by the server configuration'),
                _('Error'),
                'body' => sprintf('%s: <b>%s</b>.', _('The command could not be run'), htmlspecialchars($www['cmd'])),
                'type' => 'error'
            ),
            'index.php'
        );
...
if (isAjaxEnabled() && get_request('refresh', 'REQUEST') && get_request('refresh', 'REQUEST') != get_request(
        'frame',
        'REQUEST'
    )
) {
    ...
    printf(
        "ajDISPLAY('%s','cmd=refresh&server_id=%s&noheader=%s','%s');",
        get_request('refresh', 'REQUEST'),
        $app['server']->getIndex(),
        get_request('noheader', 'REQUEST', false, 0),
        _('Auto refresh')
    );
    echo '</script>';
}
...
if ($www['meth'] == 'ajax') {
    $www['page']->show(
        get_request('frame', 'REQUEST', false, 'BODY'),
        true,
        get_request('raw', 'REQUEST', false, false)
    );
} else {
    $www['page']->display();
}
