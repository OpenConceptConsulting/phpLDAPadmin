...
if (get_request('meth', 'REQUEST') != 'ajax') {
    ...
    if (count($request['template']->getAvailAttrs())) {
        ...
        if (count($request['template']->getAvailAttrs()) > $_SESSION[APPCONFIG]->getValue(
                'appearance',
                'max_add_attrs'
            )
        ) {
...
# The ajax addition (it is going into an existing TemplateRendered page
    ...
    printf(
        '<select name="attr" onchange="ajDISPLAY(\'%s\',\'cmd=add_value_form&server_id=%s&dn=%s&attr=\'+this.value,\'%s\',\'append\');">',
        'ADDATTR',
        $app['server']->getIndex(),
